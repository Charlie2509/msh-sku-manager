...
const removableWords = ["FC","AFC","UNITED","COACHES","PLAYERS","GIRLS","CONNECT","PIRATES","EASTBOURNE","HASTINGS","FOREST","ROW","CLUB","PRO","LEVEL","WATER","BOTTLE","GAME","DAY","HILLCREST","WESTHILL"];
const suspiciousModelWords = new Set(["3D"]);
const neverModelWords = ["ABC","BC","CLUB","PRO","LEVEL","HILLCREST","WESTHILL","WATER","BOTTLE","GAME","DAY","COACHES","PLAYERS","GIRLS","ONE","SIZE","ONE SIZE"];
const NEVER_A_MODEL_LOCAL = new Set(neverModelWords.map((w)=>w.toLowerCase()));
...
function detectMacronModelFromTitle(words){if(!words?.length) return null; const lower=words.map((w)=>w.toLowerCase().replace(/[^a-z0-9]/g,"")); for(const span of [3,2,1]) for(let i=0;i<=lower.length-span;i+=1){const slice=lower.slice(i,i+span).filter(Boolean); if(!slice.length) continue; const phrase=slice.join(" "); if(phrase==="one size") continue; if(slice.every((t)=>NEVER_A_MODEL_LOCAL.has(t))) continue; if(span===1&&NEVER_A_MODEL_LOCAL.has(slice[0])) continue; for(const c of [slice.join("-"),phrase,slice.join("")]){if(NEVER_A_MODEL_LOCAL.has(c)) continue; const ref=macronReferenceMap[c]; if(ref) return {model: words.slice(i,i+span).join(" "), modelIndices:Array.from({length:span},(_,k)=>i+k), reference:ref};}} return null;}
...
export function formatSizeLabel(size){const raw=(size??"").toString().trim(); if(!raw) return ""; const lower=raw.toLowerCase(); if(lower==="default"||lower==="default title") return "One Size"; const mapped={"s":"S","m":"M","l":"L","xl":"XL","2xl":"2XL","3xl":"3XL","4xl":"4XL","5xl":"5XL","3xs":"3XS","2xs":"2XS","xs":"XS","jnr":"JNR","snr":"SNR"}; if(mapped[lower]) return mapped[lower]; return raw.toUpperCase();}
