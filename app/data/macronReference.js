// AUTO-GENERATED from Macron_2026_Database.xlsx + pim_metadata.csv
// Multi-tone colourways like 'WHITE/SILVER' are preserved as-is.

const CATALOGUE_PENDING_SOURCE = "catalogue-pending";
const CATALOGUE_VERIFIED_SOURCE = "catalogue-verified";

const createReferenceEntry = ({
  slug, displayName, garmentType, allowedColours, aliases, source,
}) => ({
  slug, displayName, garmentType, allowedColours, aliases, source,
});

export const macronReferenceData = {
  "3-4": createReferenceEntry({
    slug: "3-4",
    displayName: "3/4",
    garmentType: "match day short",
    allowedColours: ["BLACK", "NAVY", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "accordion": createReferenceEntry({
    slug: "accordion",
    displayName: "Accordion",
    garmentType: "trousers",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "acrux-hero": createReferenceEntry({
    slug: "acrux-hero",
    displayName: "Acrux Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "WHITE"],
    aliases: ["acrux hero", "acrux-hero", "acruxhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "adamant": createReferenceEntry({
    slug: "adamant",
    displayName: "Adamant",
    garmentType: "trousers",
    allowedColours: ["GREY", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "adjustable-xi": createReferenceEntry({
    slug: "adjustable-xi",
    displayName: "Adjustable Xi",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["adjustable xi", "adjustable-xi", "adjustablexi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "advanced": createReferenceEntry({
    slug: "advanced",
    displayName: "Advanced",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "agility-ladder": createReferenceEntry({
    slug: "agility-ladder",
    displayName: "Agility Ladder",
    garmentType: "60 cm x 180 cm x 1 cm",
    allowedColours: [],
    aliases: ["agility ladder", "agility-ladder", "agilityladder"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "agnes": createReferenceEntry({
    slug: "agnes",
    displayName: "Agnes",
    garmentType: "t-shirt",
    allowedColours: ["BLUE", "ORANGE", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "airstone": createReferenceEntry({
    slug: "airstone",
    displayName: "Airstone",
    garmentType: "bermuda with zip pocket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alabaster": createReferenceEntry({
    slug: "alabaster",
    displayName: "Alabaster",
    garmentType: "t-shirt",
    allowedColours: ["BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY COLUMBIA", "NAVY WHITE", "NAVY/COLUMBIA", "NAVY/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "albatross": createReferenceEntry({
    slug: "albatross",
    displayName: "Albatross",
    garmentType: "trousers",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alchemist": createReferenceEntry({
    slug: "alchemist",
    displayName: "Alchemist",
    garmentType: "hoody",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "BOTTLE GREEN MINT", "BOTTLE GREEN/MINT", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alcyon": createReferenceEntry({
    slug: "alcyon",
    displayName: "Alcyon",
    garmentType: "t-shirt",
    allowedColours: ["WHITE BLUE", "WHITE GREEN", "WHITE RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alcyon-eco": createReferenceEntry({
    slug: "alcyon-eco",
    displayName: "Alcyon Eco",
    garmentType: "t-shirt",
    allowedColours: ["WHITE/GREEN", "WHITE/RED", "WHITE/ROYAL BLUE"],
    aliases: ["alcyon eco", "alcyon-eco", "alcyoneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alder": createReferenceEntry({
    slug: "alder",
    displayName: "Alder",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "alioth": createReferenceEntry({
    slug: "alioth",
    displayName: "Alioth",
    garmentType: "t-shirt",
    allowedColours: ["AMBER BLACK", "BLACK", "BLUE WHITE", "BLUE YELLOW", "GREEN WHITE", "GUN METAL COLUMBIA", "ORANGE BLACK", "RED WHITE", "WHITE BLACK", "WHITE BLUE", "WHITE GREEN", "WHITE RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alioth-eco": createReferenceEntry({
    slug: "alioth-eco",
    displayName: "Alioth Eco",
    garmentType: "t-shirt",
    allowedColours: ["AMBER/BLACK", "ANTHRACITE/COLUMBIA", "BLACK/DARK GREY", "GREEN/WHITE", "ORANGE", "ORANGE/BLACK", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "WHITE/BLACK", "WHITE/GREEN", "WHITE/RED", "WHITE/ROYAL BLUE"],
    aliases: ["alioth eco", "alioth-eco", "aliotheco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "all": createReferenceEntry({
    slug: "all",
    displayName: "All",
    garmentType: "bag",
    allowedColours: ["BLACK", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "all-in": createReferenceEntry({
    slug: "all-in",
    displayName: "All In",
    garmentType: "bag",
    allowedColours: [],
    aliases: ["all in", "all-in", "allin"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "allien-evo-custom": createReferenceEntry({
    slug: "allien-evo-custom",
    displayName: "Allien Evo Custom",
    garmentType: "trousers",
    allowedColours: [],
    aliases: ["allien evo custom", "allien-evo-custom", "allienevocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "alma": createReferenceEntry({
    slug: "alma",
    displayName: "Alma",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "alps": createReferenceEntry({
    slug: "alps",
    displayName: "Alps",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLACK/ANTHRACITE", "BLACK/DARK GREY", "BLUE", "NAVY", "NAVY/LIGHT NAVY", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "altair": createReferenceEntry({
    slug: "altair",
    displayName: "Altair",
    garmentType: "match day short",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "american": createReferenceEntry({
    slug: "american",
    displayName: "American",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "amethyst-hero": createReferenceEntry({
    slug: "amethyst-hero",
    displayName: "Amethyst Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: ["amethyst hero", "amethyst-hero", "amethysthero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "andes": createReferenceEntry({
    slug: "andes",
    displayName: "Andes",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "anduin": createReferenceEntry({
    slug: "anduin",
    displayName: "Anduin",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE/OFF WHITE", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GUN METAL OFF WHITE", "LILAC GUN METAL", "LILAC/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "anduril": createReferenceEntry({
    slug: "anduril",
    displayName: "Anduril",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE BLACK", "WHITE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "anora": createReferenceEntry({
    slug: "anora",
    displayName: "Anora",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "LILAC GUN METAL", "LILAC/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "anti-vibration-technology": createReferenceEntry({
    slug: "anti-vibration-technology",
    displayName: "Anti-Vibration Technology",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["anti-vibration technology", "anti-vibration-technology", "anti-vibrationtechnology"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "anubis": createReferenceEntry({
    slug: "anubis",
    displayName: "Anubis",
    garmentType: "full length zip top",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "CARDINAL", "CARDINAL/DARK CARDINAL", "GREEN", "GREEN/DARK GREEN", "NAVY", "NAVY/LIGHT NAVY", "PURPLE", "PURPLE/DARK PURPLE", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE GREY", "WHITE/LIGHT GREY", "YELLOW", "YELLOW/DARK YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "anvik-hero": createReferenceEntry({
    slug: "anvik-hero",
    displayName: "Anvik Hero",
    garmentType: "mixture",
    allowedColours: ["BLACK", "NAVY"],
    aliases: ["anvik hero", "anvik-hero", "anvikhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "aoki-eco-custom": createReferenceEntry({
    slug: "aoki-eco-custom",
    displayName: "Aoki Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["aoki eco custom", "aoki-eco-custom", "aokiecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aras": createReferenceEntry({
    slug: "aras",
    displayName: "Aras",
    garmentType: "tracksuit",
    allowedColours: ["BLACK", "GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "arcane": createReferenceEntry({
    slug: "arcane",
    displayName: "Arcane",
    garmentType: "t-shirt",
    allowedColours: ["AMBER BLACK", "BLACK GOLD", "BLACK WHITE", "BLUE WHITE", "CARDINAL WHITE", "COLUMBIA WHITE", "GREEN WHITE", "PINK BLACK", "RED WHITE", "SILVER GUN METAL", "WHITE BLUE", "WHITE RED", "WHITE SILVER", "YELLOW BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "arcane-eco": createReferenceEntry({
    slug: "arcane-eco",
    displayName: "Arcane Eco",
    garmentType: "t-shirt",
    allowedColours: ["AMBER/BLACK", "BLACK/GOLD", "BLACK/WHITE", "CARDINAL/WHITE", "COLUMBIA/WHITE", "GREEN/WHITE", "PINK/BLACK", "RED/WHITE", "ROYAL BLUE/WHITE", "SILVER/ANTHRACITE", "WHITE/RED", "WHITE/ROYAL BLUE", "WHITE/SILVER", "YELLOW/ROYAL BLUE"],
    aliases: ["arcane eco", "arcane-eco", "arcaneeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "archen": createReferenceEntry({
    slug: "archen",
    displayName: "Archen",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE", "BLACK", "BOTTLE GREEN", "GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "arctic": createReferenceEntry({
    slug: "arctic",
    displayName: "Arctic",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "argo": createReferenceEntry({
    slug: "argo",
    displayName: "Argo",
    garmentType: "t-shirt",
    allowedColours: ["NEON YELLOW BLACK", "NEON YELLOW/BLACK", "ORANGE BLACK", "ORANGE/BLACK", "PURPLE BLACK", "PURPLE/BLACK", "TURQUOISE BLACK", "TURQUOISE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ariel": createReferenceEntry({
    slug: "ariel",
    displayName: "Ariel",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "GREEN", "GUN METAL", "NAVY", "PINK", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "artemis": createReferenceEntry({
    slug: "artemis",
    displayName: "Artemis",
    garmentType: "sr",
    allowedColours: ["ANTHRACITE/LILAC", "BLACK", "BLACK/DARK GREY", "BLUE", "GUN METAL LILAC", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ash": createReferenceEntry({
    slug: "ash",
    displayName: "Ash",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "aspire-control-i": createReferenceEntry({
    slug: "aspire-control-i",
    displayName: "Aspire Control I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["aspire control i", "aspire-control-i", "aspirecontroli"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aspire-control-ii": createReferenceEntry({
    slug: "aspire-control-ii",
    displayName: "Aspire Control Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["aspire control ii", "aspire-control-ii", "aspirecontrolii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aspire-power-i": createReferenceEntry({
    slug: "aspire-power-i",
    displayName: "Aspire Power I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["aspire power i", "aspire-power-i", "aspirepoweri"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aspire-power-ii": createReferenceEntry({
    slug: "aspire-power-ii",
    displayName: "Aspire Power Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["aspire power ii", "aspire-power-ii", "aspirepowerii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aster": createReferenceEntry({
    slug: "aster",
    displayName: "Aster",
    garmentType: "bermuda with zip pocket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "athena": createReferenceEntry({
    slug: "athena",
    displayName: "Athena",
    garmentType: "full length zip top",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "athleisure": createReferenceEntry({
    slug: "athleisure",
    displayName: "Athleisure",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "atlantic-hero": createReferenceEntry({
    slug: "atlantic-hero",
    displayName: "Atlantic Hero",
    garmentType: "windbreaker",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "GREEN", "NAVY", "NEON YELLOW", "RED", "STONE GREY"],
    aliases: ["atlantic hero", "atlantic-hero", "atlantichero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "atlas": createReferenceEntry({
    slug: "atlas",
    displayName: "Atlas",
    garmentType: "sr",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE GREY", "WHITE/LIGHT GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "auke": createReferenceEntry({
    slug: "auke",
    displayName: "Auke",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "aulos": createReferenceEntry({
    slug: "aulos",
    displayName: "Aulos",
    garmentType: "polo shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLACK/DARK ANTHRACITE", "BLUE WHITE", "GREEN NEON GREEN", "GREEN/NEON GREEN", "GUN METAL", "NAVY", "NAVY/LIGHT NAVY", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE SILVER", "WHITE/SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "auriga": createReferenceEntry({
    slug: "auriga",
    displayName: "Auriga",
    garmentType: "gk training",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "aurora": createReferenceEntry({
    slug: "aurora",
    displayName: "Aurora",
    garmentType: "w - polifiber",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "auster": createReferenceEntry({
    slug: "auster",
    displayName: "Auster",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "australian": createReferenceEntry({
    slug: "australian",
    displayName: "Australian",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "axima": createReferenceEntry({
    slug: "axima",
    displayName: "Axima",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "axis": createReferenceEntry({
    slug: "axis",
    displayName: "Axis",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "axwell-eco-custom": createReferenceEntry({
    slug: "axwell-eco-custom",
    displayName: "Axwell Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["axwell eco custom", "axwell-eco-custom", "axwellecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "azlon": createReferenceEntry({
    slug: "azlon",
    displayName: "Azlon",
    garmentType: "socks",
    allowedColours: ["BLACK COLUMBIA", "BLACK/AMBER", "BLACK/COLUMBIA", "BLACK/GREEN", "BLACK/RED", "BLACK/WHITE", "BLACK/YELLOW", "BOTTLE GREEN BLACK", "BOTTLE GREEN/BLACK", "CARDINAL/COLUMBIA", "COLUMBIA/CARDINAL", "COLUMBIA/WHITE", "GREEN WHITE", "GREEN/WHITE", "LILAC/ANTHRACITE", "NAVY/COLUMBIA", "NAVY/RED", "NAVY/WHITE", "NAVY/YELLOW", "RED YELLOW", "RED/WHITE", "RED/YELLOW", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "TURQUOISE/ANTHRACITE", "YELLOW/RED", "YELLOW/ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "baldwin": createReferenceEntry({
    slug: "baldwin",
    displayName: "Baldwin",
    garmentType: "polo shirt",
    allowedColours: ["ANTHRACITE/LILAC", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GUN METAL LILAC", "NAVY", "NAVY/LIGHT NAVY", "OFF WHITE GUN METAL", "OFF WHITE/ANTHRACITE", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ball-bag": createReferenceEntry({
    slug: "ball-bag",
    displayName: "Ball Bag",
    garmentType: "bag",
    allowedColours: [],
    aliases: ["ball bag", "ball-bag", "ballbag"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ball-pump": createReferenceEntry({
    slug: "ball-pump",
    displayName: "Ball Pump",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["ball pump", "ball-pump", "ballpump"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ball-pump-needles": createReferenceEntry({
    slug: "ball-pump-needles",
    displayName: "Ball Pump Needles",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["ball pump needles", "ball-pump-needles", "ballpumpneedles"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ball-scheme": createReferenceEntry({
    slug: "ball-scheme",
    displayName: "Ball Scheme",
    garmentType: "match day",
    allowedColours: [],
    aliases: ["ball scheme", "ball-scheme", "ballscheme"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "banjo-hero": createReferenceEntry({
    slug: "banjo-hero",
    displayName: "Banjo Hero",
    garmentType: "hoody",
    allowedColours: ["BLACK", "CARDINAL", "GREY", "NAVY", "RED", "ROYAL BLUE"],
    aliases: ["banjo hero", "banjo-hero", "banjohero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "banshee": createReferenceEntry({
    slug: "banshee",
    displayName: "Banshee",
    garmentType: "match day short",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "banshee-eco": createReferenceEntry({
    slug: "banshee-eco",
    displayName: "Banshee Eco",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["banshee eco", "banshee-eco", "bansheeeco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "barber": createReferenceEntry({
    slug: "barber",
    displayName: "Barber",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "barrier": createReferenceEntry({
    slug: "barrier",
    displayName: "Barrier",
    garmentType: "gloves",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bass": createReferenceEntry({
    slug: "bass",
    displayName: "Bass",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "GREY GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bass-eco": createReferenceEntry({
    slug: "bass-eco",
    displayName: "Bass Eco",
    garmentType: "hoody",
    allowedColours: [],
    aliases: ["bass eco", "bass-eco", "basseco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bastet": createReferenceEntry({
    slug: "bastet",
    displayName: "Bastet",
    garmentType: "sr",
    allowedColours: ["ANTHRACITE/NEON SKY", "BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "GUN METAL NEON SKY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bazalt": createReferenceEntry({
    slug: "bazalt",
    displayName: "Bazalt",
    garmentType: "short",
    allowedColours: ["BLACK GREY", "BLACK/GREY", "NAVY GREY", "NAVY/GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "beat": createReferenceEntry({
    slug: "beat",
    displayName: "Beat",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "GREY", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "beginner": createReferenceEntry({
    slug: "beginner",
    displayName: "Beginner",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "beryl": createReferenceEntry({
    slug: "beryl",
    displayName: "Beryl",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bibian": createReferenceEntry({
    slug: "bibian",
    displayName: "Bibian",
    garmentType: "referee",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bibian-eco": createReferenceEntry({
    slug: "bibian-eco",
    displayName: "Bibian Eco",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["bibian eco", "bibian-eco", "bibianeco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bise": createReferenceEntry({
    slug: "bise",
    displayName: "Bise",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bismuth-hero": createReferenceEntry({
    slug: "bismuth-hero",
    displayName: "Bismuth Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "ROYAL BLUE"],
    aliases: ["bismuth hero", "bismuth-hero", "bismuthhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "blizzard": createReferenceEntry({
    slug: "blizzard",
    displayName: "Blizzard",
    garmentType: "mixture",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "boarding": createReferenceEntry({
    slug: "boarding",
    displayName: "Boarding",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "boost": createReferenceEntry({
    slug: "boost",
    displayName: "Boost",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN GOLD", "CARDINAL", "COLUMBIA", "GREEN", "GREY", "NAVY", "ORANGE", "PURPLE", "RED", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "boost-eco": createReferenceEntry({
    slug: "boost-eco",
    displayName: "Boost Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BOTTLE GREEN/GOLD", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: ["boost eco", "boost-eco", "boosteco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "boreas": createReferenceEntry({
    slug: "boreas",
    displayName: "Boreas",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "born-for-sport": createReferenceEntry({
    slug: "born-for-sport",
    displayName: "Born For Sport",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["born for sport", "born-for-sport", "bornforsport"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bottle-carrier": createReferenceEntry({
    slug: "bottle-carrier",
    displayName: "Bottle Carrier",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["bottle carrier", "bottle-carrier", "bottlecarrier"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bounce": createReferenceEntry({
    slug: "bounce",
    displayName: "Bounce",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bowls": createReferenceEntry({
    slug: "bowls",
    displayName: "Bowls",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "boxe": createReferenceEntry({
    slug: "boxe",
    displayName: "Boxe",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "breeze": createReferenceEntry({
    slug: "breeze",
    displayName: "Breeze",
    garmentType: "sr",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "broad": createReferenceEntry({
    slug: "broad",
    displayName: "Broad",
    garmentType: "t-shirt",
    allowedColours: ["OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "broad-custom": createReferenceEntry({
    slug: "broad-custom",
    displayName: "Broad Custom",
    garmentType: "match day slipover",
    allowedColours: [],
    aliases: ["broad custom", "broad-custom", "broadcustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "bromir-eco-custom": createReferenceEntry({
    slug: "bromir-eco-custom",
    displayName: "Bromir Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["bromir eco custom", "bromir-eco-custom", "bromirecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "brushed": createReferenceEntry({
    slug: "brushed",
    displayName: "Brushed",
    garmentType: "bermuda/pant",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "burst": createReferenceEntry({
    slug: "burst",
    displayName: "Burst",
    garmentType: "53 lt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "butan": createReferenceEntry({
    slug: "butan",
    displayName: "Butan",
    garmentType: "sr",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "bwp": createReferenceEntry({
    slug: "bwp",
    displayName: "Bwp",
    garmentType: "wooden bat",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "campus": createReferenceEntry({
    slug: "campus",
    displayName: "Campus",
    garmentType: "bag",
    allowedColours: ["BLACK", "NAVY", "RED", "ROYAL", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "carbon": createReferenceEntry({
    slug: "carbon",
    displayName: "Carbon",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "carbon-eco": createReferenceEntry({
    slug: "carbon-eco",
    displayName: "Carbon Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/WHITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["carbon eco", "carbon-eco", "carboneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cargo": createReferenceEntry({
    slug: "cargo",
    displayName: "Cargo",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "carol": createReferenceEntry({
    slug: "carol",
    displayName: "Carol",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cassidi-eco-custom": createReferenceEntry({
    slug: "cassidi-eco-custom",
    displayName: "Cassidi Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["cassidi eco custom", "cassidi-eco-custom", "cassidiecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "cassiopea": createReferenceEntry({
    slug: "cassiopea",
    displayName: "Cassiopea",
    garmentType: "match day short",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cassiopea-hero": createReferenceEntry({
    slug: "cassiopea-hero",
    displayName: "Cassiopea Hero",
    garmentType: "sr",
    allowedColours: [],
    aliases: ["cassiopea hero", "cassiopea-hero", "cassiopeahero"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "cello": createReferenceEntry({
    slug: "cello",
    displayName: "Cello",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "GREEN", "GREY", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cerberus": createReferenceEntry({
    slug: "cerberus",
    displayName: "Cerberus",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE WHITE", "BLUE YELLOW", "GREEN WHITE", "GUN METAL NEON YELLOW", "NAVY YELLOW", "RED BLACK", "RED WHITE", "WHITE BLACK", "WHITE BLUE", "WHITE GREEN", "WHITE RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cerberus-eco": createReferenceEntry({
    slug: "cerberus-eco",
    displayName: "Cerberus Eco",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/NEON YELLOW", "BLACK/DARK GREY", "GREEN/WHITE", "NAVY/YELLOW", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "WHITE/BLACK", "WHITE/GREEN", "WHITE/RED", "WHITE/ROYAL BLUE"],
    aliases: ["cerberus eco", "cerberus-eco", "cerberuseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cesar-hero": createReferenceEntry({
    slug: "cesar-hero",
    displayName: "Cesar Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: ["cesar hero", "cesar-hero", "cesarhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "chantico": createReferenceEntry({
    slug: "chantico",
    displayName: "Chantico",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "NEON CORAL GUN METAL", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "charm": createReferenceEntry({
    slug: "charm",
    displayName: "Charm",
    garmentType: "mini ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "charon": createReferenceEntry({
    slug: "charon",
    displayName: "Charon",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GREEN", "BLUE RED", "GUN METAL BLACK", "NAVY COLUMBIA", "NEON GREEN BLACK", "NEON YELLOW CARDINAL", "WHITE RED", "YELLOW RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "charon-eco": createReferenceEntry({
    slug: "charon-eco",
    displayName: "Charon Eco",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/BLACK/WHITE", "BLACK/GREEN/WHITE", "NAVY/COLUMBIA/WHITE", "NEON GREEN/BLACK/ROYAL BLUE", "NEON YELLOW/CARDINAL/WHITE", "ROYAL BLUE/RED/WHITE", "WHITE/RED/BLACK", "YELLOW/RED/NAVY"],
    aliases: ["charon eco", "charon-eco", "charoneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "chronos": createReferenceEntry({
    slug: "chronos",
    displayName: "Chronos",
    garmentType: "hoody",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "GREEN", "GREEN/DARK GREEN", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE", "WHITE/LIGHT GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "clarinet": createReferenceEntry({
    slug: "clarinet",
    displayName: "Clarinet",
    garmentType: "polo shirt",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "BLUE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE SILVER", "WHITE/SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "clips": createReferenceEntry({
    slug: "clips",
    displayName: "Clips",
    garmentType: "polyester - aluminium - pvc",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "cloud": createReferenceEntry({
    slug: "cloud",
    displayName: "Cloud",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "STONE GRAY", "STONE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "coldmire": createReferenceEntry({
    slug: "coldmire",
    displayName: "Coldmire",
    garmentType: "body warmer",
    allowedColours: ["BLACK", "NAVY", "STONE GRAY", "STONE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "comedy-eco": createReferenceEntry({
    slug: "comedy-eco",
    displayName: "Comedy Eco",
    garmentType: "polo shirt",
    allowedColours: ["BLACK", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: ["comedy eco", "comedy-eco", "comedyeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "compass": createReferenceEntry({
    slug: "compass",
    displayName: "Compass",
    garmentType: "bag",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "competition": createReferenceEntry({
    slug: "competition",
    displayName: "Competition",
    garmentType: "origin i",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "cone": createReferenceEntry({
    slug: "cone",
    displayName: "Cone",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "cone-flexi": createReferenceEntry({
    slug: "cone-flexi",
    displayName: "Cone Flexi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["cone flexi", "cone-flexi", "coneflexi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "conlan-eco-custom": createReferenceEntry({
    slug: "conlan-eco-custom",
    displayName: "Conlan Eco Custom",
    garmentType: "match day top singlet",
    allowedColours: [],
    aliases: ["conlan eco custom", "conlan-eco-custom", "conlanecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "corner-set": createReferenceEntry({
    slug: "corner-set",
    displayName: "Corner Set",
    garmentType: "polyester - aluminium - pvc",
    allowedColours: [],
    aliases: ["corner set", "corner-set", "cornerset"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "corver": createReferenceEntry({
    slug: "corver",
    displayName: "Corver",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "corvus": createReferenceEntry({
    slug: "corvus",
    displayName: "Corvus",
    garmentType: "t-shirt",
    allowedColours: ["GUN METAL NEON YELLOW", "NEON YELLOW BLACK", "ORANGE BLACK", "TURQUOISE BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "corvus-eco": createReferenceEntry({
    slug: "corvus-eco",
    displayName: "Corvus Eco",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/NEON YELLOW", "NEON YELLOW/BLACK", "ORANGE/BLACK", "TURQUOISE/BLACK"],
    aliases: ["corvus eco", "corvus-eco", "corvuseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cotton": createReferenceEntry({
    slug: "cotton",
    displayName: "Cotton",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK GUN METAL", "BLUE", "GREY", "NAVY", "RED", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "creed-eco-custom": createReferenceEntry({
    slug: "creed-eco-custom",
    displayName: "Creed Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["creed eco custom", "creed-eco-custom", "creedecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "crew": createReferenceEntry({
    slug: "crew",
    displayName: "Crew",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "GREEN", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cricket-catalogue": createReferenceEntry({
    slug: "cricket-catalogue",
    displayName: "Cricket Catalogue",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["cricket catalogue", "cricket-catalogue", "cricketcatalogue"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "crowe": createReferenceEntry({
    slug: "crowe",
    displayName: "Crowe",
    garmentType: "t-shirt",
    allowedColours: ["WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cruise": createReferenceEntry({
    slug: "cruise",
    displayName: "Cruise",
    garmentType: "bag",
    allowedColours: ["BLACK", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "curium": createReferenceEntry({
    slug: "curium",
    displayName: "Curium",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "GREEN", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cuzco": createReferenceEntry({
    slug: "cuzco",
    displayName: "Cuzco",
    garmentType: "tracksuit",
    allowedColours: ["BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "BLUE WHITE", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "NEON CORAL GUN METAL", "NEON CORAL/ANTHRACITE", "NEON GREEN GREEN", "NEON GREEN/GREEN", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cygnus": createReferenceEntry({
    slug: "cygnus",
    displayName: "Cygnus",
    garmentType: "t-shirt",
    allowedColours: ["GUN METAL BLACK", "NEON GREEN BLACK", "NEON YELLOW BLACK", "PURPLE BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cygnus-eco": createReferenceEntry({
    slug: "cygnus-eco",
    displayName: "Cygnus Eco",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/BLACK", "NEON GREEN/BLACK", "NEON YELLOW/BLACK", "PURPLE/BLACK"],
    aliases: ["cygnus eco", "cygnus-eco", "cygnuseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "cymbal": createReferenceEntry({
    slug: "cymbal",
    displayName: "Cymbal",
    garmentType: "trousers",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dacite-hero": createReferenceEntry({
    slug: "dacite-hero",
    displayName: "Dacite Hero",
    garmentType: "bermuda/pant",
    allowedColours: ["BLACK", "NAVY"],
    aliases: ["dacite hero", "dacite-hero", "dacitehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dahlia": createReferenceEntry({
    slug: "dahlia",
    displayName: "Dahlia",
    garmentType: "trousers",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "daily": createReferenceEntry({
    slug: "daily",
    displayName: "Daily",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dance": createReferenceEntry({
    slug: "dance",
    displayName: "Dance",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE", "BLACK", "BLACK GUN METAL", "BLACK/ANTHRACITE", "BLUE", "GREY", "GREY GUN METAL", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "danube-hero": createReferenceEntry({
    slug: "danube-hero",
    displayName: "Danube Hero",
    garmentType: "sr",
    allowedColours: ["ANTHRACITE", "BLACK", "GUN METAL", "NAVY"],
    aliases: ["danube hero", "danube-hero", "danubehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dart": createReferenceEntry({
    slug: "dart",
    displayName: "Dart",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "defender": createReferenceEntry({
    slug: "defender",
    displayName: "Defender",
    garmentType: "trousers",
    allowedColours: ["BLACK ORANGE", "BLUE RED", "GREY", "NAVY RED", "RED BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "degree-xh": createReferenceEntry({
    slug: "degree-xh",
    displayName: "Degree Xh",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["degree xh", "degree-xh", "degreexh"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "degree-xi": createReferenceEntry({
    slug: "degree-xi",
    displayName: "Degree Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["degree xi", "degree-xi", "degreexi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "delaney": createReferenceEntry({
    slug: "delaney",
    displayName: "Delaney",
    garmentType: "trousers",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "delcourt": createReferenceEntry({
    slug: "delcourt",
    displayName: "Delcourt",
    garmentType: "t-shirt",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "denali": createReferenceEntry({
    slug: "denali",
    displayName: "Denali",
    garmentType: "windbreaker",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "denver-hero": createReferenceEntry({
    slug: "denver-hero",
    displayName: "Denver Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "CARDINAL WHITE", "CARDINAL/WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: ["denver hero", "denver-hero", "denverhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "design-su-misura-per-seguire-i-tuoi-movimenti": createReferenceEntry({
    slug: "design-su-misura-per-seguire-i-tuoi-movimenti",
    displayName: "Design Su Misura Per Seguire I Tuoi Movimenti",
    garmentType: "performance",
    allowedColours: [],
    aliases: ["design su misura per seguire i tuoi movimenti", "design-su-misura-per-seguire-i-tuoi-movimenti", "designsumisuraperseguireituoimovimenti"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "desna-hero": createReferenceEntry({
    slug: "desna-hero",
    displayName: "Desna Hero",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE", "BLACK", "GUN METAL", "NAVY"],
    aliases: ["desna hero", "desna-hero", "desnahero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "deva": createReferenceEntry({
    slug: "deva",
    displayName: "Deva",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "GREEN", "NAVY", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dew-xi": createReferenceEntry({
    slug: "dew-xi",
    displayName: "Dew Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["dew xi", "dew-xi", "dewxi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dexif": createReferenceEntry({
    slug: "dexif",
    displayName: "Dexif",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dexif-custom": createReferenceEntry({
    slug: "dexif-custom",
    displayName: "Dexif Custom",
    garmentType: "socks",
    allowedColours: [],
    aliases: ["dexif custom", "dexif-custom", "dexifcustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "diamond": createReferenceEntry({
    slug: "diamond",
    displayName: "Diamond",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "GREY", "NAVY", "RED", "WHITE", "WHITE NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "diamond-evo": createReferenceEntry({
    slug: "diamond-evo",
    displayName: "Diamond Evo",
    garmentType: "trousers",
    allowedColours: ["BLACK", "GREY", "NAVY", "RED", "ROYAL BLUE", "WHITE", "WHITE/NAVY"],
    aliases: ["diamond evo", "diamond-evo", "diamondevo"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "discovery": createReferenceEntry({
    slug: "discovery",
    displayName: "Discovery",
    garmentType: "t-shirt",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dise-o-a-medida-para-adaptarse-a-tus-movimientos": createReferenceEntry({
    slug: "dise-o-a-medida-para-adaptarse-a-tus-movimientos",
    displayName: "Diseño A Medida Para Adaptarse A Tus Movimientos",
    garmentType: "performance",
    allowedColours: [],
    aliases: ["diseño a medida para adaptarse a tus movimientos", "diseño-a-medida-para-adaptarse-a-tus-movimientos", "diseñoamedidaparaadaptarseatusmovimientos"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "doja-eco-custom": createReferenceEntry({
    slug: "doja-eco-custom",
    displayName: "Doja Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["doja eco custom", "doja-eco-custom", "dojaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dolly": createReferenceEntry({
    slug: "dolly",
    displayName: "Dolly",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "NEON YELLOW BLACK", "NEON YELLOW/BLACK", "RED BLACK", "RED/BLACK", "ROYAL BLUE/BLACK", "TURQUOISE BLACK", "TURQUOISE/BLACK", "WHITE BLACK", "WHITE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dolly-eco-custom": createReferenceEntry({
    slug: "dolly-eco-custom",
    displayName: "Dolly Eco Custom",
    garmentType: "eco tank",
    allowedColours: [],
    aliases: ["dolly eco custom", "dolly-eco-custom", "dollyecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dolomite": createReferenceEntry({
    slug: "dolomite",
    displayName: "Dolomite",
    garmentType: "trousers",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "doom": createReferenceEntry({
    slug: "doom",
    displayName: "Doom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dora": createReferenceEntry({
    slug: "dora",
    displayName: "Dora",
    garmentType: "tracksuit",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "SILVER GUN METAL", "SILVER/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dorian": createReferenceEntry({
    slug: "dorian",
    displayName: "Dorian",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "NEON YELLOW BLACK", "NEON YELLOW/BLACK", "RED BLACK", "RED/BLACK", "ROYAL BLUE/BLACK", "TURQUOISE BLACK", "TURQUOISE/BLACK", "WHITE BLACK", "WHITE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dorian-eco-custom": createReferenceEntry({
    slug: "dorian-eco-custom",
    displayName: "Dorian Eco Custom",
    garmentType: "eco singlet",
    allowedColours: [],
    aliases: ["dorian eco custom", "dorian-eco-custom", "dorianecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "draco": createReferenceEntry({
    slug: "draco",
    displayName: "Draco",
    garmentType: "polo shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "COLUMBIA WHITE", "COLUMBIA/WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "WHITE GREY", "WHITE/GREY", "YELLOW WHITE", "YELLOW/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "draco-hero": createReferenceEntry({
    slug: "draco-hero",
    displayName: "Draco Hero",
    garmentType: "polo shirt",
    allowedColours: ["ANTHRACITE/WHITE", "BLACK WHITE", "BLACK/WHITE", "BOTTLE GREEN/GOLD", "CARDINAL/WHITE", "COLUMBIA/WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE/GREY", "YELLOW/WHITE"],
    aliases: ["draco hero", "draco-hero", "dracohero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "drake": createReferenceEntry({
    slug: "drake",
    displayName: "Drake",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "NAVY", "ORANGE", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dravid-eco-custom": createReferenceEntry({
    slug: "dravid-eco-custom",
    displayName: "Dravid Eco Custom",
    garmentType: "trousers",
    allowedColours: [],
    aliases: ["dravid eco custom", "dravid-eco-custom", "dravidecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dravite": createReferenceEntry({
    slug: "dravite",
    displayName: "Dravite",
    garmentType: "match day short",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dravite-eco": createReferenceEntry({
    slug: "dravite-eco",
    displayName: "Dravite Eco",
    garmentType: "shorts",
    allowedColours: ["BLACK", "NAVY"],
    aliases: ["dravite eco", "dravite-eco", "draviteeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "drizzle": createReferenceEntry({
    slug: "drizzle",
    displayName: "Drizzle",
    garmentType: "mesh lined windbreaker",
    allowedColours: ["BLACK", "BLUE", "COLUMBIA", "GREEN", "NAVY", "NEON YELLOW", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "drum": createReferenceEntry({
    slug: "drum",
    displayName: "Drum",
    garmentType: "trousers",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "dryad": createReferenceEntry({
    slug: "dryad",
    displayName: "Dryad",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "LILAC GUN METAL", "LILAC/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "duality": createReferenceEntry({
    slug: "duality",
    displayName: "Duality",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dynamic": createReferenceEntry({
    slug: "dynamic",
    displayName: "Dynamic",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE", "BLACK", "BLACK/ANTHRACITE", "GREY", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "dynamite-kit": createReferenceEntry({
    slug: "dynamite-kit",
    displayName: "Dynamite Kit",
    garmentType: "bat and ball set",
    allowedColours: [],
    aliases: ["dynamite kit", "dynamite-kit", "dynamitekit"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "eagle": createReferenceEntry({
    slug: "eagle",
    displayName: "Eagle",
    garmentType: "bermuda",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "earthquake-xi": createReferenceEntry({
    slug: "earthquake-xi",
    displayName: "Earthquake Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["earthquake xi", "earthquake-xi", "earthquakexi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ease": createReferenceEntry({
    slug: "ease",
    displayName: "Ease",
    garmentType: "t-shirt",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "easy": createReferenceEntry({
    slug: "easy",
    displayName: "Easy",
    garmentType: "basketball ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "eblana": createReferenceEntry({
    slug: "eblana",
    displayName: "Eblana",
    garmentType: "jacket",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "echoes": createReferenceEntry({
    slug: "echoes",
    displayName: "Echoes",
    garmentType: "polo shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "BOTTLE GREEN MINT", "BOTTLE GREEN/MINT", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "elastic-belt": createReferenceEntry({
    slug: "elastic-belt",
    displayName: "Elastic Belt",
    garmentType: "evo",
    allowedColours: [],
    aliases: ["elastic belt", "elastic-belt", "elasticbelt"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "elbrus": createReferenceEntry({
    slug: "elbrus",
    displayName: "Elbrus",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "STONE GRAY", "STONE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "electro": createReferenceEntry({
    slug: "electro",
    displayName: "Electro",
    garmentType: "hoody",
    allowedColours: ["BLACK/ANTHRACITE", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL/DARK CARDINAL", "NAVY/LIGHT NAVY", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "SILVER/DARK SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "elevate": createReferenceEntry({
    slug: "elevate",
    displayName: "Elevate",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "elite-control-i": createReferenceEntry({
    slug: "elite-control-i",
    displayName: "Elite Control I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["elite control i", "elite-control-i", "elitecontroli"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "elite-control-ii": createReferenceEntry({
    slug: "elite-control-ii",
    displayName: "Elite Control Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["elite control ii", "elite-control-ii", "elitecontrolii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "elite-power-i": createReferenceEntry({
    slug: "elite-power-i",
    displayName: "Elite Power I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["elite power i", "elite-power-i", "elitepoweri"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "elite-power-ii": createReferenceEntry({
    slug: "elite-power-ii",
    displayName: "Elite Power Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["elite power ii", "elite-power-ii", "elitepowerii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "elm": createReferenceEntry({
    slug: "elm",
    displayName: "Elm",
    garmentType: "bermuda/pant",
    allowedColours: ["BLACK", "NAVY", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "elves": createReferenceEntry({
    slug: "elves",
    displayName: "Elves",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "CARDINAL COLUMBIA", "CARDINAL/COLUMBIA", "GREEN WHITE", "GREEN/WHITE", "ORANGE BLACK", "ORANGE/BLACK", "PURPLE WHITE", "PURPLE/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE", "YELLOW BLUE", "YELLOW/ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "elyon-eco-custom": createReferenceEntry({
    slug: "elyon-eco-custom",
    displayName: "Elyon Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["elyon eco custom", "elyon-eco-custom", "elyonecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "endurance": createReferenceEntry({
    slug: "endurance",
    displayName: "Endurance",
    garmentType: "socks",
    allowedColours: ["BLACK GREY", "BLACK NEON CORAL", "BLACK NEON YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "enhance": createReferenceEntry({
    slug: "enhance",
    displayName: "Enhance",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "etesian": createReferenceEntry({
    slug: "etesian",
    displayName: "Etesian",
    garmentType: "shoes",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "exert": createReferenceEntry({
    slug: "exert",
    displayName: "Exert",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "f500": createReferenceEntry({
    slug: "f500",
    displayName: "F500",
    garmentType: "w - double",
    allowedColours: ["BLUE WHITE", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fenders": createReferenceEntry({
    slug: "fenders",
    displayName: "Fenders",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE/LILAC", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "GUN METAL LILAC", "NAVY", "NAVY/LIGHT NAVY", "OFF WHITE GUN METAL", "OFF WHITE/ANTHRACITE", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fern": createReferenceEntry({
    slug: "fern",
    displayName: "Fern",
    garmentType: "bermuda",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ferry": createReferenceEntry({
    slug: "ferry",
    displayName: "Ferry",
    garmentType: "mixture",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "fiddle": createReferenceEntry({
    slug: "fiddle",
    displayName: "Fiddle",
    garmentType: "t-shirt",
    allowedColours: ["BLACK OFF WHITE", "BLACK/OFF WHITE", "BLUE OFF WHITE", "NAVY OFF WHITE", "NAVY/OFF WHITE", "RED OFF WHITE", "RED/OFF WHITE", "ROYAL BLUE/OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fifa-standard-for-official-competitions": createReferenceEntry({
    slug: "fifa-standard-for-official-competitions",
    displayName: "Fifa Standard For Official Competitions",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["fifa standard for official competitions", "fifa-standard-for-official-competitions", "fifastandardforofficialcompetitions"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "finlay-hero": createReferenceEntry({
    slug: "finlay-hero",
    displayName: "Finlay Hero",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "NAVY WHITE"],
    aliases: ["finlay hero", "finlay-hero", "finlayhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fiona": createReferenceEntry({
    slug: "fiona",
    displayName: "Fiona",
    garmentType: "bra",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fitness-mat": createReferenceEntry({
    slug: "fitness-mat",
    displayName: "Fitness Mat",
    garmentType: "60 cm x 180 cm x 1 cm",
    allowedColours: [],
    aliases: ["fitness mat", "fitness-mat", "fitnessmat"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "fixed": createReferenceEntry({
    slug: "fixed",
    displayName: "Fixed",
    garmentType: "socks",
    allowedColours: ["BLACK", "NAVY", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "flags": createReferenceEntry({
    slug: "flags",
    displayName: "Flags",
    garmentType: "polyester - aluminium - pvc",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "flamenco": createReferenceEntry({
    slug: "flamenco",
    displayName: "Flamenco",
    garmentType: "polo shirt",
    allowedColours: ["BLACK/WHITE", "GREEN/WHITE", "GREY/ANTHRACITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "flat-marker-rubber": createReferenceEntry({
    slug: "flat-marker-rubber",
    displayName: "Flat Marker Rubber",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["flat marker rubber", "flat-marker-rubber", "flatmarkerrubber"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "flight": createReferenceEntry({
    slug: "flight",
    displayName: "Flight",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "flow": createReferenceEntry({
    slug: "flow",
    displayName: "Flow",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "flower": createReferenceEntry({
    slug: "flower",
    displayName: "Flower",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "folk": createReferenceEntry({
    slug: "folk",
    displayName: "Folk",
    garmentType: "polo shirt",
    allowedColours: ["BLACK RED", "BLACK WHITE", "BLACK/RED", "BLUE WHITE", "GREEN BLACK", "GREEN/BLACK", "NAVY RED", "NAVY WHITE", "NAVY YELLOW", "NAVY/RED", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "fraser": createReferenceEntry({
    slug: "fraser",
    displayName: "Fraser",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK/DARK GREY", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "CARDINAL/DARK CARDINAL", "GREEN WHITE", "GREEN/DARK GREEN", "GREEN/WHITE", "NAVY WHITE", "NAVY YELLOW", "NAVY/LIGHT NAVY", "NAVY/WHITE", "NAVY/YELLOW", "NEON SKY GUN METAL", "NEON SKY/ANTHRACITE", "ORANGE/DARK ORANGE", "PURPLE/DARK PURPLE", "RED WHITE", "RED/DARK RED", "RED/WHITE", "ROYAL BLUE/DARK ROYAL BLUE", "ROYAL BLUE/WHITE", "YELLOW BLACK", "YELLOW/BLACK", "YELLOW/DARK YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "freon": createReferenceEntry({
    slug: "freon",
    displayName: "Freon",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE", "YELLOW WHITE", "YELLOW/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "freyr": createReferenceEntry({
    slug: "freyr",
    displayName: "Freyr",
    garmentType: "hoody",
    allowedColours: ["BLACK", "BLUE", "GREEN", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "frisk": createReferenceEntry({
    slug: "frisk",
    displayName: "Frisk",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "NEON SKY BLACK", "NEON SKY BLUE/BLACK", "NEON YELLOW BLACK", "NEON YELLOW/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "frostmoor": createReferenceEntry({
    slug: "frostmoor",
    displayName: "Frostmoor",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE BLACK", "WHITE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "full": createReferenceEntry({
    slug: "full",
    displayName: "Full",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "BLUE", "GREY GUN METAL", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gades-eco-custom": createReferenceEntry({
    slug: "gades-eco-custom",
    displayName: "Gades Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["gades eco custom", "gades-eco-custom", "gadesecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "gadreel": createReferenceEntry({
    slug: "gadreel",
    displayName: "Gadreel",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "GREY", "NAVY", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gadreel-eco": createReferenceEntry({
    slug: "gadreel-eco",
    displayName: "Gadreel Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NAVY", "WHITE"],
    aliases: ["gadreel eco", "gadreel-eco", "gadreeleco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "galax": createReferenceEntry({
    slug: "galax",
    displayName: "Galax",
    garmentType: "shorts",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON CORAL", "NEON GREEN", "NEON SKY", "NEON YELLOW", "ORANGE", "PINK", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "galler": createReferenceEntry({
    slug: "galler",
    displayName: "Galler",
    garmentType: "mixture",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gallium-custom": createReferenceEntry({
    slug: "gallium-custom",
    displayName: "Gallium Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["gallium custom", "gallium-custom", "galliumcustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "gaston-hero": createReferenceEntry({
    slug: "gaston-hero",
    displayName: "Gaston Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: ["gaston hero", "gaston-hero", "gastonhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gate": createReferenceEntry({
    slug: "gate",
    displayName: "Gate",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ghible-eco-custom": createReferenceEntry({
    slug: "ghible-eco-custom",
    displayName: "Ghible Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["ghible eco custom", "ghible-eco-custom", "ghibleecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "gila": createReferenceEntry({
    slug: "gila",
    displayName: "Gila",
    garmentType: "sr",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gloves": createReferenceEntry({
    slug: "gloves",
    displayName: "Gloves",
    garmentType: "mixture",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "golem": createReferenceEntry({
    slug: "golem",
    displayName: "Golem",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "BLUE WHITE", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "NEON CORAL GUN METAL", "NEON CORAL/ANTHRACITE", "NEON GREEN GREEN", "NEON GREEN/GREEN", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE SILVER", "WHITE/SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "golpeo": createReferenceEntry({
    slug: "golpeo",
    displayName: "Golpeo",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "grace": createReferenceEntry({
    slug: "grace",
    displayName: "Grace",
    garmentType: "t-shirt",
    allowedColours: ["WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "granite": createReferenceEntry({
    slug: "granite",
    displayName: "Granite",
    garmentType: "r. training",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "greece-eco-custom": createReferenceEntry({
    slug: "greece-eco-custom",
    displayName: "Greece Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["greece eco custom", "greece-eco-custom", "greeceecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "gripfit": createReferenceEntry({
    slug: "gripfit",
    displayName: "Gripfit",
    garmentType: "socks",
    allowedColours: ["BLACK", "BLUE", "GREEN", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ground": createReferenceEntry({
    slug: "ground",
    displayName: "Ground",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ground-xi": createReferenceEntry({
    slug: "ground-xi",
    displayName: "Ground Xi",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "ROYAL BLUE"],
    aliases: ["ground xi", "ground-xi", "groundxi"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "guardians-evo": createReferenceEntry({
    slug: "guardians-evo",
    displayName: "Guardians Evo",
    garmentType: "shinguards",
    allowedColours: [],
    aliases: ["guardians evo", "guardians-evo", "guardiansevo"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "guardians-pro": createReferenceEntry({
    slug: "guardians-pro",
    displayName: "Guardians Pro",
    garmentType: "shinguards",
    allowedColours: [],
    aliases: ["guardians pro", "guardians-pro", "guardianspro"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "guitar": createReferenceEntry({
    slug: "guitar",
    displayName: "Guitar",
    garmentType: "trousers",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "gym": createReferenceEntry({
    slug: "gym",
    displayName: "Gym",
    garmentType: "bag",
    allowedColours: ["BLACK", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "gyor": createReferenceEntry({
    slug: "gyor",
    displayName: "Gyor",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "halley": createReferenceEntry({
    slug: "halley",
    displayName: "Halley",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/WHITE", "COLUMBIA/NAVY", "GREEN/WHITE", "NAVY/WHITE", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/WHITE", "SILVER/ANTHRACITE", "WHITE/SILVER", "YELLOW/ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hangar": createReferenceEntry({
    slug: "hangar",
    displayName: "Hangar",
    garmentType: "bag",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hanna-hero": createReferenceEntry({
    slug: "hanna-hero",
    displayName: "Hanna Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: ["hanna hero", "hanna-hero", "hannahero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "harp": createReferenceEntry({
    slug: "harp",
    displayName: "Harp",
    garmentType: "trousers",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "harris-eco-custom": createReferenceEntry({
    slug: "harris-eco-custom",
    displayName: "Harris Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["harris eco custom", "harris-eco-custom", "harrisecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "harvey": createReferenceEntry({
    slug: "harvey",
    displayName: "Harvey",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "havel": createReferenceEntry({
    slug: "havel",
    displayName: "Havel",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON YELLOW", "ORANGE", "RED", "ROYAL BLUE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hb": createReferenceEntry({
    slug: "hb",
    displayName: "Hb",
    garmentType: "one size",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "heavy": createReferenceEntry({
    slug: "heavy",
    displayName: "Heavy",
    garmentType: "tracksuit",
    allowedColours: ["BLACK", "GREY", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "helia-evo-custom": createReferenceEntry({
    slug: "helia-evo-custom",
    displayName: "Helia Evo Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["helia evo custom", "helia-evo-custom", "heliaevocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "heliodor": createReferenceEntry({
    slug: "heliodor",
    displayName: "Heliodor",
    garmentType: "bermuda/pant",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "helmet": createReferenceEntry({
    slug: "helmet",
    displayName: "Helmet",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "helmet-xe": createReferenceEntry({
    slug: "helmet-xe",
    displayName: "Helmet Xe",
    garmentType: "wr headgear",
    allowedColours: ["BLACK", "RED"],
    aliases: ["helmet xe", "helmet-xe", "helmetxe"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "heracles": createReferenceEntry({
    slug: "heracles",
    displayName: "Heracles",
    garmentType: "polifiber",
    allowedColours: ["ANTHRACITE", "BLACK", "GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hero": createReferenceEntry({
    slug: "hero",
    displayName: "Hero",
    garmentType: "sr",
    allowedColours: ["BLACK", "GREY", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hero-pro-control-i": createReferenceEntry({
    slug: "hero-pro-control-i",
    displayName: "Hero Pro Control I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["hero pro control i", "hero-pro-control-i", "heroprocontroli"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hero-pro-control-ii": createReferenceEntry({
    slug: "hero-pro-control-ii",
    displayName: "Hero Pro Control Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["hero pro control ii", "hero-pro-control-ii", "heroprocontrolii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hero-pro-power-i": createReferenceEntry({
    slug: "hero-pro-power-i",
    displayName: "Hero Pro Power I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["hero pro power i", "hero-pro-power-i", "heropropoweri"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hero-pro-power-ii": createReferenceEntry({
    slug: "hero-pro-power-ii",
    displayName: "Hero Pro Power Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["hero pro power ii", "hero-pro-power-ii", "heropropowerii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "heroes": createReferenceEntry({
    slug: "heroes",
    displayName: "Heroes",
    garmentType: "backpack",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hestia": createReferenceEntry({
    slug: "hestia",
    displayName: "Hestia",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "himalaya": createReferenceEntry({
    slug: "himalaya",
    displayName: "Himalaya",
    garmentType: "jacket",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hit": createReferenceEntry({
    slug: "hit",
    displayName: "Hit",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hit-mouth-guard": createReferenceEntry({
    slug: "hit-mouth-guard",
    displayName: "Hit Mouth Guard",
    garmentType: "mouth guard",
    allowedColours: [],
    aliases: ["hit mouth guard", "hit-mouth-guard", "hitmouthguard"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hobbs": createReferenceEntry({
    slug: "hobbs",
    displayName: "Hobbs",
    garmentType: "t-shirt",
    allowedColours: ["OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hobbs-eco-custom": createReferenceEntry({
    slug: "hobbs-eco-custom",
    displayName: "Hobbs Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["hobbs eco custom", "hobbs-eco-custom", "hobbsecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hollow": createReferenceEntry({
    slug: "hollow",
    displayName: "Hollow",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "MINT BOTTLE GREEN", "MINT/BOTTLE GREEN", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "holly": createReferenceEntry({
    slug: "holly",
    displayName: "Holly",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON GREEN", "NEON YELLOW", "ORANGE", "PINK", "PURPLE", "RED", "ROYAL BLUE", "TURQUOISE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hooded": createReferenceEntry({
    slug: "hooded",
    displayName: "Hooded",
    garmentType: "windbreaker",
    allowedColours: ["BLACK GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hoodie": createReferenceEntry({
    slug: "hoodie",
    displayName: "Hoodie",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "NAVY", "RED", "SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hoops": createReferenceEntry({
    slug: "hoops",
    displayName: "Hoops",
    garmentType: "socks",
    allowedColours: ["BLACK/RED", "BLACK/WHITE", "BLACK/YELLOW", "BLUE RED", "COLUMBIA/WHITE", "GREEN BLACK", "GREEN/BLACK", "GREEN/WHITE", "NAVY/COLUMBIA", "NAVY/RED", "RED/WHITE", "ROYAL BLUE/RED", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "horn": createReferenceEntry({
    slug: "horn",
    displayName: "Horn",
    garmentType: "hoody",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "CARDINAL", "CARDINAL/DARK CARDINAL NEON YELLOW/DARK NEON YELLOW", "GREEN", "GREEN/DARK GREEN", "NAVY", "NAVY/LIGHT NAVY", "NEON CORAL NEON CORAL", "NEON CORAL/DARK NEON CORAL", "NEON SKY NEON SKY", "NEON SKY/DARK NEON SKY", "NEON YELLOW NEON YELLOW", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE", "TURQUOISE/DARK TURQUOISE", "WHITE GREY", "WHITE/LIGHT GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "houston-eco-custom": createReferenceEntry({
    slug: "houston-eco-custom",
    displayName: "Houston Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["houston eco custom", "houston-eco-custom", "houstonecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "howlite-eco-custom": createReferenceEntry({
    slug: "howlite-eco-custom",
    displayName: "Howlite Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["howlite eco custom", "howlite-eco-custom", "howliteecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "howlite-hero": createReferenceEntry({
    slug: "howlite-hero",
    displayName: "Howlite Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "NAVY", "RED", "WHITE"],
    aliases: ["howlite hero", "howlite-hero", "howlitehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hurdles": createReferenceEntry({
    slug: "hurdles",
    displayName: "Hurdles",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hutton": createReferenceEntry({
    slug: "hutton",
    displayName: "Hutton",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NAVY", "OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "hutton-eco-custom": createReferenceEntry({
    slug: "hutton-eco-custom",
    displayName: "Hutton Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["hutton eco custom", "hutton-eco-custom", "huttonecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "hydrogen-hero": createReferenceEntry({
    slug: "hydrogen-hero",
    displayName: "Hydrogen Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY"],
    aliases: ["hydrogen hero", "hydrogen-hero", "hydrogenhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "icefang": createReferenceEntry({
    slug: "icefang",
    displayName: "Icefang",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "idaho": createReferenceEntry({
    slug: "idaho",
    displayName: "Idaho",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLUE WHITE", "CARDINAL WHITE", "GREEN WHITE", "NAVY WHITE", "NAVY YELLOW", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "idmon": createReferenceEntry({
    slug: "idmon",
    displayName: "Idmon",
    garmentType: "t-shirt",
    allowedColours: ["BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "NAVY COLUMBIA", "NAVY RED", "NAVY WHITE", "NAVY/COLUMBIA", "NAVY/RED", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "WHITE GREEN", "WHITE/GREEN"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "igloo": createReferenceEntry({
    slug: "igloo",
    displayName: "Igloo",
    garmentType: "sr",
    allowedColours: ["BLACK", "BLUE", "NAVY", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ignite-i": createReferenceEntry({
    slug: "ignite-i",
    displayName: "Ignite I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["ignite i", "ignite-i", "ignitei"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ike-hero": createReferenceEntry({
    slug: "ike-hero",
    displayName: "Ike Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: ["ike hero", "ike-hero", "ikehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "improve": createReferenceEntry({
    slug: "improve",
    displayName: "Improve",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "inning": createReferenceEntry({
    slug: "inning",
    displayName: "Inning",
    garmentType: "t-shirt",
    allowedColours: ["BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "intermediate": createReferenceEntry({
    slug: "intermediate",
    displayName: "Intermediate",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "interval": createReferenceEntry({
    slug: "interval",
    displayName: "Interval",
    garmentType: "socks",
    allowedColours: ["BLACK GREY", "BLACK NEON CORAL", "BLACK NEON YELLOW", "BLACK/GREY", "BLACK/NEON CORAL", "BLACK/NEON YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "inuit": createReferenceEntry({
    slug: "inuit",
    displayName: "Inuit",
    garmentType: "sr",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "NAVY", "NAVY/LIGHT NAVY", "SILVER", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "iolite": createReferenceEntry({
    slug: "iolite",
    displayName: "Iolite",
    garmentType: "sr",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "irma": createReferenceEntry({
    slug: "irma",
    displayName: "Irma",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "NEON YELLOW BLACK", "RED BLACK", "TURQUOISE BLACK", "WHITE BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "irma-eco-custom": createReferenceEntry({
    slug: "irma-eco-custom",
    displayName: "Irma Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["irma eco custom", "irma-eco-custom", "irmaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "iron": createReferenceEntry({
    slug: "iron",
    displayName: "Iron",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE NAVY", "WHITE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "isen": createReferenceEntry({
    slug: "isen",
    displayName: "Isen",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE/DARK ANTHRACITE", "BLACK", "BLUE", "BOTTLE GREEN GOLD", "BOTTLE GREEN/GOLD", "CARDINAL", "GREEN", "GUN METAL", "NAVY", "NEON SKY NEON SKY", "NEON SKY/DARK NEON SKY", "ORANGE", "PURPLE", "RED", "TURQUOISE", "TURQUOISE/DARK TURQUOISE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ivan": createReferenceEntry({
    slug: "ivan",
    displayName: "Ivan",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "NEON YELLOW BLACK", "NEON YELLOW/BLACK", "RED BLACK", "RED/BLACK", "ROYAL BLUE/BLACK", "TURQUOISE BLACK", "TURQUOISE/BLACK", "WHITE BLACK", "WHITE/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ivan-eco-custom": createReferenceEntry({
    slug: "ivan-eco-custom",
    displayName: "Ivan Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["ivan eco custom", "ivan-eco-custom", "ivanecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ivy": createReferenceEntry({
    slug: "ivy",
    displayName: "Ivy",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "izel": createReferenceEntry({
    slug: "izel",
    displayName: "Izel",
    garmentType: "tracksuit",
    allowedColours: ["BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "BLUE WHITE", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "NEON CORAL GUN METAL", "NEON CORAL/ANTHRACITE", "NEON GREEN GREEN", "NEON GREEN/GREEN", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "jasper": createReferenceEntry({
    slug: "jasper",
    displayName: "Jasper",
    garmentType: "t-shirt",
    allowedColours: ["BLACK AMBER", "BLACK GREY", "BLACK/AMBER", "BLACK/GREY", "BLUE YELLOW", "NAVY WHITE", "NAVY/WHITE", "RED BLACK", "RED WHITE", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "javelin": createReferenceEntry({
    slug: "javelin",
    displayName: "Javelin",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "jazz": createReferenceEntry({
    slug: "jazz",
    displayName: "Jazz",
    garmentType: "polo shirt",
    allowedColours: ["ANTHRACITE", "BLUE", "GREY", "GUN METAL", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "joanne-eco-custom": createReferenceEntry({
    slug: "joanne-eco-custom",
    displayName: "Joanne Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["joanne eco custom", "joanne-eco-custom", "joanneecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "jolt": createReferenceEntry({
    slug: "jolt",
    displayName: "Jolt",
    garmentType: "match day ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "jotnar": createReferenceEntry({
    slug: "jotnar",
    displayName: "Jotnar",
    garmentType: "polifiber",
    allowedColours: ["BLACK", "BLUE", "NAVY", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "journey": createReferenceEntry({
    slug: "journey",
    displayName: "Journey",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "julien-eco-custom": createReferenceEntry({
    slug: "julien-eco-custom",
    displayName: "Julien Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["julien eco custom", "julien-eco-custom", "julienecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "junior": createReferenceEntry({
    slug: "junior",
    displayName: "Junior",
    garmentType: "anti-",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "kansas": createReferenceEntry({
    slug: "kansas",
    displayName: "Kansas",
    garmentType: "match day short",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "ORANGE WHITE", "RED WHITE", "WHITE", "YELLOW WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "kansas-eco": createReferenceEntry({
    slug: "kansas-eco",
    displayName: "Kansas Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/WHITE", "GREEN/WHITE", "NAVY/WHITE", "ORANGE/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE", "YELLOW/WHITE"],
    aliases: ["kansas eco", "kansas-eco", "kansaseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "kateryn": createReferenceEntry({
    slug: "kateryn",
    displayName: "Kateryn",
    garmentType: "referee",
    allowedColours: ["BLACK", "NEON RED", "NEON SKY", "NEON YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "kateryn-eco": createReferenceEntry({
    slug: "kateryn-eco",
    displayName: "Kateryn Eco",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["kateryn eco", "kateryn-eco", "kateryneco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "kelvin": createReferenceEntry({
    slug: "kelvin",
    displayName: "Kelvin",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "kesil": createReferenceEntry({
    slug: "kesil",
    displayName: "Kesil",
    garmentType: "sr",
    allowedColours: ["BLACK", "GREEN", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "kleffa-eco-custom": createReferenceEntry({
    slug: "kleffa-eco-custom",
    displayName: "Kleffa Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["kleffa eco custom", "kleffa-eco-custom", "kleffaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "kneepads-and": createReferenceEntry({
    slug: "kneepads-and",
    displayName: "Kneepads And",
    garmentType: "unknown",
    allowedColours: ["BLACK", "ROYAL BLUE", "WHITE"],
    aliases: ["kneepads and", "kneepads-and", "kneepadsand"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "kolyma": createReferenceEntry({
    slug: "kolyma",
    displayName: "Kolyma",
    garmentType: "training top",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "krypton": createReferenceEntry({
    slug: "krypton",
    displayName: "Krypton",
    garmentType: "w - match day short",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "krypton-evo-hero": createReferenceEntry({
    slug: "krypton-evo-hero",
    displayName: "Krypton Evo Hero",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["krypton evo hero", "krypton-evo-hero", "kryptonevohero"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "la-microfibra-amica-della-pelle": createReferenceEntry({
    slug: "la-microfibra-amica-della-pelle",
    displayName: "La Microfibra Amica Della Pelle",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["la microfibra amica della pelle", "la-microfibra-amica-della-pelle", "lamicrofibraamicadellapelle"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lancer": createReferenceEntry({
    slug: "lancer",
    displayName: "Lancer",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lanny-eco-custom": createReferenceEntry({
    slug: "lanny-eco-custom",
    displayName: "Lanny Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["lanny eco custom", "lanny-eco-custom", "lannyecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lapis": createReferenceEntry({
    slug: "lapis",
    displayName: "Lapis",
    garmentType: "shorts",
    allowedColours: ["BLACK", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "legion": createReferenceEntry({
    slug: "legion",
    displayName: "Legion",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "BLUE RED", "BLUE WHITE", "COLUMBIA WHITE", "COLUMBIA/WHITE", "GREEN BLACK", "GREEN WHITE", "GREEN/BLACK", "GREEN/WHITE", "NAVY RED", "NAVY/RED", "RED BLACK", "RED WHITE", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/BLACK", "ROYAL BLUE/RED", "ROYAL BLUE/WHITE", "WHITE BLACK", "WHITE/BLACK", "YELLOW BLACK", "YELLOW/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "leisure": createReferenceEntry({
    slug: "leisure",
    displayName: "Leisure",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "GREEN", "GREY", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "lena": createReferenceEntry({
    slug: "lena",
    displayName: "Lena",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "NAVY", "NAVY/LIGHT NAVY", "NEON SKY NEON SKY", "NEON SKY/DARK NEON SKY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE", "TURQUOISE/DARK TURQUOISE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "leona": createReferenceEntry({
    slug: "leona",
    displayName: "Leona",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "leonidas-frequency": createReferenceEntry({
    slug: "leonidas-frequency",
    displayName: "Leonidas Frequency",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["leonidas frequency", "leonidas-frequency", "leonidasfrequency"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "leonidas-light-frequency": createReferenceEntry({
    slug: "leonidas-light-frequency",
    displayName: "Leonidas Light Frequency",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["leonidas light frequency", "leonidas-light-frequency", "leonidaslightfrequency"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lily": createReferenceEntry({
    slug: "lily",
    displayName: "Lily",
    garmentType: "t-shirt",
    allowedColours: ["BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "RED WHITE", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "lily-eco": createReferenceEntry({
    slug: "lily-eco",
    displayName: "Lily Eco",
    garmentType: "t-shirt",
    allowedColours: ["GREEN/WHITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE/SILVER"],
    aliases: ["lily eco", "lily-eco", "lilyeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "linesman": createReferenceEntry({
    slug: "linesman",
    displayName: "Linesman",
    garmentType: "polyester - aluminium - pvc",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "load": createReferenceEntry({
    slug: "load",
    displayName: "Load",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lotus": createReferenceEntry({
    slug: "lotus",
    displayName: "Lotus",
    garmentType: "shorts",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "lute": createReferenceEntry({
    slug: "lute",
    displayName: "Lute",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/ANTHRACITE", "NAVY/LIGHT NAVY", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE/SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "lutz": createReferenceEntry({
    slug: "lutz",
    displayName: "Lutz",
    garmentType: "mixture",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "luzern": createReferenceEntry({
    slug: "luzern",
    displayName: "Luzern",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLUE", "NAVY", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "lyngen": createReferenceEntry({
    slug: "lyngen",
    displayName: "Lyngen",
    garmentType: "mesh lined windbreaker",
    allowedColours: ["BLACK", "BLACK WHITE", "BLUE WHITE", "GREEN", "GREEN WHITE", "NAVY", "NAVY WHITE", "RED", "RED WHITE", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "maas": createReferenceEntry({
    slug: "maas",
    displayName: "Maas",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE", "BLACK", "GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "maira": createReferenceEntry({
    slug: "maira",
    displayName: "Maira",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "NEON SKY GUN METAL", "NEON SKY/ANTHRACITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "major": createReferenceEntry({
    slug: "major",
    displayName: "Major",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "makalu": createReferenceEntry({
    slug: "makalu",
    displayName: "Makalu",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "maple": createReferenceEntry({
    slug: "maple",
    displayName: "Maple",
    garmentType: "wooden bat",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "maracas": createReferenceEntry({
    slug: "maracas",
    displayName: "Maracas",
    garmentType: "bermuda",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "marker": createReferenceEntry({
    slug: "marker",
    displayName: "Marker",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "marple": createReferenceEntry({
    slug: "marple",
    displayName: "Marple",
    garmentType: "sr",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "match-day-baseball": createReferenceEntry({
    slug: "match-day-baseball",
    displayName: "Match Day Baseball",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["match day baseball", "match-day-baseball", "matchdaybaseball"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "match-day-cricket": createReferenceEntry({
    slug: "match-day-cricket",
    displayName: "Match Day Cricket",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["match day cricket", "match-day-cricket", "matchdaycricket"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "matt-eco-custom": createReferenceEntry({
    slug: "matt-eco-custom",
    displayName: "Matt Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["matt eco custom", "matt-eco-custom", "mattecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "maxi": createReferenceEntry({
    slug: "maxi",
    displayName: "Maxi",
    garmentType: "bag",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "maxi-academy-evo": createReferenceEntry({
    slug: "maxi-academy-evo",
    displayName: "Maxi Academy Evo",
    garmentType: "unknown",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL BLUE"],
    aliases: ["maxi academy evo", "maxi-academy-evo", "maxiacademyevo"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "maxi-path": createReferenceEntry({
    slug: "maxi-path",
    displayName: "Maxi Path",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["maxi path", "maxi-path", "maxipath"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "medal": createReferenceEntry({
    slug: "medal",
    displayName: "Medal",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "melody": createReferenceEntry({
    slug: "melody",
    displayName: "Melody",
    garmentType: "polo shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GREY", "WHITE/GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "mendez": createReferenceEntry({
    slug: "mendez",
    displayName: "Mendez",
    garmentType: "referee",
    allowedColours: ["BLACK GOLD", "NEON SKY BLACK", "NEON YELLOW BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "mendez-eco": createReferenceEntry({
    slug: "mendez-eco",
    displayName: "Mendez Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK/GOLD", "NEON SKY BLUE/BLACK", "NEON YELLOW/BLACK"],
    aliases: ["mendez eco", "mendez-eco", "mendezeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "mesa-hero": createReferenceEntry({
    slug: "mesa-hero",
    displayName: "Mesa Hero",
    garmentType: "shorts",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON GREEN", "NEON YELLOW", "ORANGE", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: ["mesa hero", "mesa-hero", "mesahero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "metel": createReferenceEntry({
    slug: "metel",
    displayName: "Metel",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "miley-eco-custom": createReferenceEntry({
    slug: "miley-eco-custom",
    displayName: "Miley Eco Custom",
    garmentType: "eco brief",
    allowedColours: [],
    aliases: ["miley eco custom", "miley-eco-custom", "mileyecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "minerva": createReferenceEntry({
    slug: "minerva",
    displayName: "Minerva",
    garmentType: "trousers",
    allowedColours: ["BLACK", "GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "mistral": createReferenceEntry({
    slug: "mistral",
    displayName: "Mistral",
    garmentType: "sr",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "morpheus": createReferenceEntry({
    slug: "morpheus",
    displayName: "Morpheus",
    garmentType: "microfiber",
    allowedColours: ["ANTHRACITE/LILAC", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GUN METAL LILAC", "NAVY", "NAVY/LIGHT NAVY", "OFF WHITE GUN METAL", "OFF WHITE/ANTHRACITE", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "mythril": createReferenceEntry({
    slug: "mythril",
    displayName: "Mythril",
    garmentType: "bermuda/pant",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nacido-para-el-deporte": createReferenceEntry({
    slug: "nacido-para-el-deporte",
    displayName: "Nacido Para El Deporte",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["nacido para el deporte", "nacido-para-el-deporte", "nacidoparaeldeporte"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "naga": createReferenceEntry({
    slug: "naga",
    displayName: "Naga",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "narvik": createReferenceEntry({
    slug: "narvik",
    displayName: "Narvik",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nash": createReferenceEntry({
    slug: "nash",
    displayName: "Nash",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLUE WHITE", "GREEN BLACK", "NAVY WHITE", "NAVY YELLOW", "RED WHITE", "SILVER GUN METAL", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nasser": createReferenceEntry({
    slug: "nasser",
    displayName: "Nasser",
    garmentType: "referee",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nasser-eco": createReferenceEntry({
    slug: "nasser-eco",
    displayName: "Nasser Eco",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["nasser eco", "nasser-eco", "nassereco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nata-per-lo-sport": createReferenceEntry({
    slug: "nata-per-lo-sport",
    displayName: "Nata Per Lo Sport",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["nata per lo sport", "nata-per-lo-sport", "nataperlosport"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "natiu-eco-custom": createReferenceEntry({
    slug: "natiu-eco-custom",
    displayName: "Natiu Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["natiu eco custom", "natiu-eco-custom", "natiuecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nebula": createReferenceEntry({
    slug: "nebula",
    displayName: "Nebula",
    garmentType: "shorts",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "GREEN", "GUN METAL", "NAVY", "NEON CORAL", "PINK", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "neckar": createReferenceEntry({
    slug: "neckar",
    displayName: "Neckar",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "NAVY WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nemesis": createReferenceEntry({
    slug: "nemesis",
    displayName: "Nemesis",
    garmentType: "full length zip top",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "ORANGE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nen": createReferenceEntry({
    slug: "nen",
    displayName: "Nen",
    garmentType: "sr",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nepri-hero": createReferenceEntry({
    slug: "nepri-hero",
    displayName: "Nepri Hero",
    garmentType: "polifiber",
    allowedColours: ["ANTHRACITE", "BLACK", "BLACK WHITE", "GUN METAL WHITE", "NAVY", "NAVY WHITE"],
    aliases: ["nepri hero", "nepri-hero", "neprihero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nerissa-eco-custom": createReferenceEntry({
    slug: "nerissa-eco-custom",
    displayName: "Nerissa Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["nerissa eco custom", "nerissa-eco-custom", "nerissaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nerium": createReferenceEntry({
    slug: "nerium",
    displayName: "Nerium",
    garmentType: "sr",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "netball": createReferenceEntry({
    slug: "netball",
    displayName: "Netball",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nevel": createReferenceEntry({
    slug: "nevel",
    displayName: "Nevel",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "BLUE", "BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GREEN", "GREEN/DARK GREEN", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "WHITE SILVER", "WHITE/SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "next-gen-ii": createReferenceEntry({
    slug: "next-gen-ii",
    displayName: "Next Gen Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["next gen ii", "next-gen-ii", "nextgenii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nistro": createReferenceEntry({
    slug: "nistro",
    displayName: "Nistro",
    garmentType: "sr",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nitro": createReferenceEntry({
    slug: "nitro",
    displayName: "Nitro",
    garmentType: "socks",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nitro-ii": createReferenceEntry({
    slug: "nitro-ii",
    displayName: "Nitro Ii",
    garmentType: "socks",
    allowedColours: ["BLACK"],
    aliases: ["nitro ii", "nitro-ii", "nitroii"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "nobium": createReferenceEntry({
    slug: "nobium",
    displayName: "Nobium",
    garmentType: "pro basketball ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "northland": createReferenceEntry({
    slug: "northland",
    displayName: "Northland",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "notus": createReferenceEntry({
    slug: "notus",
    displayName: "Notus",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "nyra-eco-custom": createReferenceEntry({
    slug: "nyra-eco-custom",
    displayName: "Nyra Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["nyra eco custom", "nyra-eco-custom", "nyraecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "oak": createReferenceEntry({
    slug: "oak",
    displayName: "Oak",
    garmentType: "shorts",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON GREEN", "NEON YELLOW", "ORANGE", "PINK", "PURPLE", "RED", "ROYAL BLUE", "TURQUOISE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "oasis-xg": createReferenceEntry({
    slug: "oasis-xg",
    displayName: "Oasis Xg",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["oasis xg", "oasis-xg", "oasisxg"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "obsidian-eco-custom": createReferenceEntry({
    slug: "obsidian-eco-custom",
    displayName: "Obsidian Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["obsidian eco custom", "obsidian-eco-custom", "obsidianecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "odette-hero": createReferenceEntry({
    slug: "odette-hero",
    displayName: "Odette Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK"],
    aliases: ["odette hero", "odette-hero", "odettehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "odin": createReferenceEntry({
    slug: "odin",
    displayName: "Odin",
    garmentType: "full length zip top",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "BOTTLE GREEN MINT", "BOTTLE GREEN/MINT", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "offroad": createReferenceEntry({
    slug: "offroad",
    displayName: "Offroad",
    garmentType: "socks",
    allowedColours: ["BLACK/ANTHRACITE", "NEON CORAL/WHITE", "NEON YELLOW/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ophelia": createReferenceEntry({
    slug: "ophelia",
    displayName: "Ophelia",
    garmentType: "brief",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "oregon-eco-custom": createReferenceEntry({
    slug: "oregon-eco-custom",
    displayName: "Oregon Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["oregon eco custom", "oregon-eco-custom", "oregonecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "origin-i": createReferenceEntry({
    slug: "origin-i",
    displayName: "Origin I",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["origin i", "origin-i", "origini"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "orpheus": createReferenceEntry({
    slug: "orpheus",
    displayName: "Orpheus",
    garmentType: "microfiber",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "orube-eco-custom": createReferenceEntry({
    slug: "orube-eco-custom",
    displayName: "Orube Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["orube eco custom", "orube-eco-custom", "orubeecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "osmium-evo-eco-custom": createReferenceEntry({
    slug: "osmium-evo-eco-custom",
    displayName: "Osmium Evo Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["osmium evo eco custom", "osmium-evo-eco-custom", "osmiumevoecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "osmium-hero": createReferenceEntry({
    slug: "osmium-hero",
    displayName: "Osmium Hero",
    garmentType: "shorts",
    allowedColours: ["BLACK", "BLUE", "NAVY"],
    aliases: ["osmium hero", "osmium-hero", "osmiumhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "otago-eco-custom": createReferenceEntry({
    slug: "otago-eco-custom",
    displayName: "Otago Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["otago eco custom", "otago-eco-custom", "otagoecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "over": createReferenceEntry({
    slug: "over",
    displayName: "Over",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "oxide-hero": createReferenceEntry({
    slug: "oxide-hero",
    displayName: "Oxide Hero",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "GREEN", "NAVY", "RED", "WHITE", "YELLOW"],
    aliases: ["oxide hero", "oxide-hero", "oxidehero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "palladium": createReferenceEntry({
    slug: "palladium",
    displayName: "Palladium",
    garmentType: "volleyball ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "pants": createReferenceEntry({
    slug: "pants",
    displayName: "Pants",
    garmentType: "w - bermuda/pant",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "paros": createReferenceEntry({
    slug: "paros",
    displayName: "Paros",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "passenger": createReferenceEntry({
    slug: "passenger",
    displayName: "Passenger",
    garmentType: "bag",
    allowedColours: ["BLACK", "NAVY", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "path": createReferenceEntry({
    slug: "path",
    displayName: "Path",
    garmentType: "bag",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "peak": createReferenceEntry({
    slug: "peak",
    displayName: "Peak",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "pearl-eco-custom": createReferenceEntry({
    slug: "pearl-eco-custom",
    displayName: "Pearl Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["pearl eco custom", "pearl-eco-custom", "pearlecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "pepper": createReferenceEntry({
    slug: "pepper",
    displayName: "Pepper",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "peridot": createReferenceEntry({
    slug: "peridot",
    displayName: "Peridot",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLACK AMBER", "BLACK/AMBER", "BLACK/DARK GREY", "BLUE", "BLUE YELLOW", "BOTTLE GREEN", "CARDINAL AMBER", "COLUMBIA BLACK", "COLUMBIA/BLACK", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED BLACK", "RED/BLACK", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "ROYAL BLUE/YELLOW", "WHITE", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "petanque": createReferenceEntry({
    slug: "petanque",
    displayName: "Petanque",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "piave": createReferenceEntry({
    slug: "piave",
    displayName: "Piave",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "NEON SKY GUN METAL", "NEON SKY/ANTHRACITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "YELLOW BLACK", "YELLOW/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "pilot": createReferenceEntry({
    slug: "pilot",
    displayName: "Pilot",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "pinto-eco-custom": createReferenceEntry({
    slug: "pinto-eco-custom",
    displayName: "Pinto Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["pinto eco custom", "pinto-eco-custom", "pintoecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "platinum": createReferenceEntry({
    slug: "platinum",
    displayName: "Platinum",
    garmentType: "w - match day short",
    allowedColours: ["BLUE", "GREEN", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "platinum-eco": createReferenceEntry({
    slug: "platinum-eco",
    displayName: "Platinum Eco",
    garmentType: "shorts",
    allowedColours: ["GREEN/WHITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["platinum eco", "platinum-eco", "platinumeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "plaza-eco-custom": createReferenceEntry({
    slug: "plaza-eco-custom",
    displayName: "Plaza Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["plaza eco custom", "plaza-eco-custom", "plazaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "plutonium": createReferenceEntry({
    slug: "plutonium",
    displayName: "Plutonium",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "ORANGE WHITE", "RED WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "plutonium-eco": createReferenceEntry({
    slug: "plutonium-eco",
    displayName: "Plutonium Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/DARK GREY/WHITE", "GREEN/DARK GREEN/WHITE", "NAVY/LIGHT NAVY/WHITE", "ORANGE/DARK ORANGE/WHITE", "RED/DARK RED/WHITE", "ROYAL BLUE/DARK ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["plutonium eco", "plutonium-eco", "plutoniumeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "pole": createReferenceEntry({
    slug: "pole",
    displayName: "Pole",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "pole-spike": createReferenceEntry({
    slug: "pole-spike",
    displayName: "Pole Spike",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["pole spike", "pole-spike", "polespike"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "polis": createReferenceEntry({
    slug: "polis",
    displayName: "Polis",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/RED", "BLACK/WHITE", "CARDINAL/COLUMBIA", "COLUMBIA/WHITE", "GOLD/WHITE", "GREEN/RED", "GREEN/WHITE", "NAVY/RED", "NAVY/WHITE", "NAVY/YELLOW", "NEON YELLOW/ANTHRACITE", "ORANGE/WHITE", "PURPLE/WHITE", "RED/BLACK", "RED/WHITE", "RED/YELLOW", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "WHITE/ANTHRACITE", "YELLOW/BLACK", "YELLOW/GREEN", "YELLOW/ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ponnet": createReferenceEntry({
    slug: "ponnet",
    displayName: "Ponnet",
    garmentType: "referee",
    allowedColours: ["BLACK", "NEON RED", "NEON SKY", "NEON YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "ponnet-eco": createReferenceEntry({
    slug: "ponnet-eco",
    displayName: "Ponnet Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "NEON RED", "NEON SKY", "NEON YELLOW"],
    aliases: ["ponnet eco", "ponnet-eco", "ponneteco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "poplar": createReferenceEntry({
    slug: "poplar",
    displayName: "Poplar",
    garmentType: "kneepads",
    allowedColours: ["BLACK", "BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "poseidon": createReferenceEntry({
    slug: "poseidon",
    displayName: "Poseidon",
    garmentType: "full length zip top",
    allowedColours: ["ANTHRACITE/LILAC", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GUN METAL LILAC", "NAVY", "NAVY/LIGHT NAVY", "OFF WHITE GUN METAL", "OFF WHITE/ANTHRACITE", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "poseidon-ii": createReferenceEntry({
    slug: "poseidon-ii",
    displayName: "Poseidon Ii",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["poseidon ii", "poseidon-ii", "poseidonii"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "poseidon-premium": createReferenceEntry({
    slug: "poseidon-premium",
    displayName: "Poseidon Premium",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["poseidon premium", "poseidon-premium", "poseidonpremium"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "practice": createReferenceEntry({
    slug: "practice",
    displayName: "Practice+",
    garmentType: "bibs",
    allowedColours: ["BLUE", "NEON GREEN", "NEON YELLOW", "ORANGE", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "praia-hero": createReferenceEntry({
    slug: "praia-hero",
    displayName: "Praia Hero",
    garmentType: "mesh lined windbreaker",
    allowedColours: ["BLACK", "BLACK GREY", "BLUE GREY", "GREEN", "GREEN GREY", "NAVY", "NAVY GREY", "RED", "RED GREY", "ROYAL BLUE"],
    aliases: ["praia hero", "praia-hero", "praiahero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "pro": createReferenceEntry({
    slug: "pro",
    displayName: "Pro",
    garmentType: "socks",
    allowedColours: ["BLACK", "BLUE", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "pro-grip-hero": createReferenceEntry({
    slug: "pro-grip-hero",
    displayName: "Pro Grip Hero",
    garmentType: "socks",
    allowedColours: ["BLACK", "RED", "ROYAL BLUE", "WHITE"],
    aliases: ["pro grip hero", "pro-grip-hero", "progriphero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "professional": createReferenceEntry({
    slug: "professional",
    displayName: "Professional",
    garmentType: "matt",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "prometheus": createReferenceEntry({
    slug: "prometheus",
    displayName: "Prometheus",
    garmentType: "microfiber",
    allowedColours: ["ANTHRACITE/NEON CORAL", "BLACK", "BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "BLUE WHITE", "GREEN NEON GREEN", "GREEN/NEON GREEN", "GUN METAL NEON CORAL", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "prometium": createReferenceEntry({
    slug: "prometium",
    displayName: "Prometium",
    garmentType: "basketball ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "propus": createReferenceEntry({
    slug: "propus",
    displayName: "Propus",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "BLUE CARDINAL", "BLUE WHITE", "COLUMBIA WHITE", "NAVY RED", "RED BLACK", "RED WHITE", "WHITE BLACK", "YELLOW BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "propus-eco": createReferenceEntry({
    slug: "propus-eco",
    displayName: "Propus Eco",
    garmentType: "t-shirt",
    allowedColours: ["COLUMBIA/WHITE", "NAVY/RED", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/BLACK", "ROYAL BLUE/CARDINAL", "ROYAL BLUE/WHITE", "WHITE/BLACK", "YELLOW/ROYAL BLUE"],
    aliases: ["propus eco", "propus-eco", "propuseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rackets": createReferenceEntry({
    slug: "rackets",
    displayName: "Rackets",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rackets-scheme": createReferenceEntry({
    slug: "rackets-scheme",
    displayName: "Rackets Scheme",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["rackets scheme", "rackets-scheme", "racketsscheme"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "raijin": createReferenceEntry({
    slug: "raijin",
    displayName: "Raijin",
    garmentType: "full length zip top",
    allowedColours: ["ANTHRACITE/NEON SKY", "BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "CARDINAL WHITE", "GREEN WHITE", "GREEN/WHITE", "GUN METAL NEON SKY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rama": createReferenceEntry({
    slug: "rama",
    displayName: "Rama",
    garmentType: "sr",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "WHITE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rap": createReferenceEntry({
    slug: "rap",
    displayName: "Rap",
    garmentType: "polo shirt",
    allowedColours: ["ANTHRACITE/NEON SKY", "BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "CARDINAL/WHITE", "GREEN WHITE", "GREEN/WHITE", "GUN METAL NEON SKY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE", "YELLOW BLACK", "YELLOW/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rayon": createReferenceEntry({
    slug: "rayon",
    displayName: "Rayon",
    garmentType: "socks",
    allowedColours: ["BOTTLE GREEN", "GREEN"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "recommendation": createReferenceEntry({
    slug: "recommendation",
    displayName: "Recommendation",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "record": createReferenceEntry({
    slug: "record",
    displayName: "Record",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "relay": createReferenceEntry({
    slug: "relay",
    displayName: "Relay",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rescue": createReferenceEntry({
    slug: "rescue",
    displayName: "Rescue",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rhenium": createReferenceEntry({
    slug: "rhenium",
    displayName: "Rhenium",
    garmentType: "basketball ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rhodium": createReferenceEntry({
    slug: "rhodium",
    displayName: "Rhodium",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "RED WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rhodium-eco": createReferenceEntry({
    slug: "rhodium-eco",
    displayName: "Rhodium Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/DARK GREY/WHITE", "GREEN/DARK GREEN/WHITE", "NAVY/LIGHT NAVY/WHITE", "RED/DARK RED/WHITE", "ROYAL BLUE/DARK ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["rhodium eco", "rhodium-eco", "rhodiumeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "richard": createReferenceEntry({
    slug: "richard",
    displayName: "Richard",
    garmentType: "match day short",
    allowedColours: ["BLACK", "NAVY", "OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "richards": createReferenceEntry({
    slug: "richards",
    displayName: "Richards",
    garmentType: "trousers",
    allowedColours: ["BLACK", "NAVY", "OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "richards-eco-custom": createReferenceEntry({
    slug: "richards-eco-custom",
    displayName: "Richards Eco Custom",
    garmentType: "trousers",
    allowedColours: [],
    aliases: ["richards eco custom", "richards-eco-custom", "richardsecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rigel-hero": createReferenceEntry({
    slug: "rigel-hero",
    displayName: "Rigel Hero",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON GREEN", "NEON YELLOW", "ORANGE", "PINK", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: ["rigel hero", "rigel-hero", "rigelhero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rise": createReferenceEntry({
    slug: "rise",
    displayName: "Rise",
    garmentType: "trousers",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rivendell": createReferenceEntry({
    slug: "rivendell",
    displayName: "Rivendell",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rivet": createReferenceEntry({
    slug: "rivet",
    displayName: "Rivet",
    garmentType: "mixture",
    allowedColours: ["BLACK", "BLUE", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rocket": createReferenceEntry({
    slug: "rocket",
    displayName: "Rocket",
    garmentType: "bag",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "rookie": createReferenceEntry({
    slug: "rookie",
    displayName: "Rookie",
    garmentType: "unknown",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "round": createReferenceEntry({
    slug: "round",
    displayName: "Round",
    garmentType: "socks",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "PURPLE", "RED", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "round-evo": createReferenceEntry({
    slug: "round-evo",
    displayName: "Round Evo",
    garmentType: "socks",
    allowedColours: ["BLACK", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: ["round evo", "round-evo", "roundevo"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "route": createReferenceEntry({
    slug: "route",
    displayName: "Route",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "roxy-eco-custom": createReferenceEntry({
    slug: "roxy-eco-custom",
    displayName: "Roxy Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["roxy eco custom", "roxy-eco-custom", "roxyecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "rubber-pole-base": createReferenceEntry({
    slug: "rubber-pole-base",
    displayName: "Rubber Pole Base",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["rubber pole base", "rubber-pole-base", "rubberpolebase"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ruby": createReferenceEntry({
    slug: "ruby",
    displayName: "Ruby",
    garmentType: "r. training",
    allowedColours: ["BLACK", "BLACK AMBER", "BLACK GREEN", "BLACK RED", "BLACK/GREEN", "BLACK/RED", "BLUE YELLOW", "NAVY", "NAVY COLUMBIA", "NAVY/COLUMBIA", "RED", "ROYAL BLUE/YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sage": createReferenceEntry({
    slug: "sage",
    displayName: "Sage",
    garmentType: "kneepads",
    allowedColours: ["BLACK", "BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sage-evo": createReferenceEntry({
    slug: "sage-evo",
    displayName: "Sage Evo",
    garmentType: "kneepads",
    allowedColours: [],
    aliases: ["sage evo", "sage-evo", "sageevo"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "saiph": createReferenceEntry({
    slug: "saiph",
    displayName: "Saiph",
    garmentType: "gk training",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "salzach": createReferenceEntry({
    slug: "salzach",
    displayName: "Salzach",
    garmentType: "training top",
    allowedColours: ["BLACK", "BLUE", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sarez": createReferenceEntry({
    slug: "sarez",
    displayName: "Sarez",
    garmentType: "sr",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sasha": createReferenceEntry({
    slug: "sasha",
    displayName: "Sasha",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "sceptrum": createReferenceEntry({
    slug: "sceptrum",
    displayName: "Sceptrum",
    garmentType: "t-shirt",
    allowedColours: ["BLUE BLACK", "BLUE RED", "BLUE WHITE", "COLUMBIA WHITE", "COLUMBIA/WHITE", "GREEN BLACK", "GREEN WHITE", "GREEN/BLACK", "GREEN/WHITE", "RED BLACK", "RED WHITE", "RED/BLACK", "RED/WHITE", "ROYAL BLUE/BLACK", "ROYAL BLUE/RED", "ROYAL BLUE/WHITE", "WHITE BLACK", "WHITE/BLACK", "YELLOW BLACK", "YELLOW RED", "YELLOW/BLACK", "YELLOW/RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "scorpio": createReferenceEntry({
    slug: "scorpio",
    displayName: "Scorpio",
    garmentType: "t-shirt",
    allowedColours: ["NEON YELLOW BLACK", "ORANGE BLACK", "PURPLE BLACK", "TURQUOISE BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "scorpio-eco": createReferenceEntry({
    slug: "scorpio-eco",
    displayName: "Scorpio Eco",
    garmentType: "t-shirt",
    allowedColours: ["NEON YELLOW/BLACK", "ORANGE/BLACK", "PURPLE/BLACK", "TURQUOISE/BLACK"],
    aliases: ["scorpio eco", "scorpio-eco", "scorpioeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "seraphine-eco-custom": createReferenceEntry({
    slug: "seraphine-eco-custom",
    displayName: "Seraphine Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["seraphine eco custom", "seraphine-eco-custom", "seraphineecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "shedir": createReferenceEntry({
    slug: "shedir",
    displayName: "Shedir",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "CARDINAL/WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "NEON SKY GUN METAL", "NEON SKY/ANTHRACITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE", "YELLOW BLACK", "YELLOW/BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "shen": createReferenceEntry({
    slug: "shen",
    displayName: "Shen",
    garmentType: "match day short",
    allowedColours: ["BLACK GREEN", "BLACK ORANGE", "BLACK RED", "BLACK WHITE", "BLACK YELLOW", "BLUE WHITE", "BLUE YELLOW", "CARDINAL WHITE", "GREEN WHITE", "NAVY RED", "NAVY WHITE", "NAVY YELLOW", "RED WHITE", "WHITE BLACK", "WHITE BLUE", "WHITE RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "shen-eco": createReferenceEntry({
    slug: "shen-eco",
    displayName: "Shen Eco",
    garmentType: "shorts",
    allowedColours: ["BLACK/GREEN", "BLACK/ORANGE", "BLACK/RED", "BLACK/WHITE", "BLACK/YELLOW", "CARDINAL/WHITE", "GREEN/WHITE", "NAVY/RED", "NAVY/WHITE", "NAVY/YELLOW", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "WHITE/BLACK", "WHITE/RED", "WHITE/ROYAL BLUE"],
    aliases: ["shen eco", "shen-eco", "sheneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "shirt": createReferenceEntry({
    slug: "shirt",
    displayName: "Shirt",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/WHITE", "NAVY/WHITE", "ORANGE/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "shoes": createReferenceEntry({
    slug: "shoes",
    displayName: "Shoes",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "shorts": createReferenceEntry({
    slug: "shorts",
    displayName: "Shorts",
    garmentType: "shorts",
    allowedColours: ["BLACK/WHITE", "BLACK/YELLOW", "CARDINAL/WHITE", "GREEN/WHITE", "NAVY/WHITE", "NAVY/YELLOW", "ORANGE/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "shuttle": createReferenceEntry({
    slug: "shuttle",
    displayName: "Shuttle",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "shyla-eco-custom": createReferenceEntry({
    slug: "shyla-eco-custom",
    displayName: "Shyla Eco Custom",
    garmentType: "eco crop top",
    allowedColours: [],
    aliases: ["shyla eco custom", "shyla-eco-custom", "shylaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "simoon": createReferenceEntry({
    slug: "simoon",
    displayName: "Simoon",
    garmentType: "competition ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "skara": createReferenceEntry({
    slug: "skara",
    displayName: "Skara",
    garmentType: "match day short",
    allowedColours: ["BLACK", "BLACK RED", "BLACK WHITE", "BLUE", "BLUE WHITE", "BLUE YELLOW", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "RED", "RED WHITE", "TURQUOISE", "WHITE", "WHITE BLACK", "WHITE BLUE", "WHITE GREEN", "WHITE RED", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "skara-eco": createReferenceEntry({
    slug: "skara-eco",
    displayName: "Skara Eco",
    garmentType: "shorts",
    allowedColours: ["ANTHRACITE", "BLACK", "BLACK/RED", "BLACK/WHITE", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "RED", "RED/WHITE", "ROYAL BLUE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "TURQUOISE", "WHITE", "WHITE/BLACK", "WHITE/GREEN", "WHITE/RED", "WHITE/ROYAL BLUE", "YELLOW"],
    aliases: ["skara eco", "skara-eco", "skaraeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "skarn": createReferenceEntry({
    slug: "skarn",
    displayName: "Skarn",
    garmentType: "r. training",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "skat": createReferenceEntry({
    slug: "skat",
    displayName: "Skat",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "CARDINAL WHITE", "CARDINAL/WHITE", "NAVY WHITE", "NAVY/WHITE", "NEON SKY GUN METAL", "NEON SKY/ANTHRACITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "skill": createReferenceEntry({
    slug: "skill",
    displayName: "Skill",
    garmentType: "socks",
    allowedColours: ["BLACK", "COLUMBIA", "GREEN", "NAVY", "RED", "ROYAL", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "skitti-eco-custom": createReferenceEntry({
    slug: "skitti-eco-custom",
    displayName: "Skitti Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["skitti eco custom", "skitti-eco-custom", "skittiecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "snow": createReferenceEntry({
    slug: "snow",
    displayName: "Snow",
    garmentType: "mixture",
    allowedColours: ["BLACK", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "softball": createReferenceEntry({
    slug: "softball",
    displayName: "Softball",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "solstice-xh": createReferenceEntry({
    slug: "solstice-xh",
    displayName: "Solstice Xh",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["solstice xh", "solstice-xh", "solsticexh"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "solstice-xi": createReferenceEntry({
    slug: "solstice-xi",
    displayName: "Solstice Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["solstice xi", "solstice-xi", "solsticexi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "spark": createReferenceEntry({
    slug: "spark",
    displayName: "Spark",
    garmentType: "mini ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "speed-ring": createReferenceEntry({
    slug: "speed-ring",
    displayName: "Speed Ring",
    garmentType: "ø 50 cm available in yellow",
    allowedColours: [],
    aliases: ["speed ring", "speed-ring", "speedring"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "sphinx": createReferenceEntry({
    slug: "sphinx",
    displayName: "Sphinx",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/WHITE", "NEON CORAL GUN METAL", "NEON CORAL/ANTHRACITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sprint": createReferenceEntry({
    slug: "sprint",
    displayName: "Sprint",
    garmentType: "socks",
    allowedColours: ["BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "PURPLE", "RED", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sprint-evo": createReferenceEntry({
    slug: "sprint-evo",
    displayName: "Sprint Evo",
    garmentType: "socks",
    allowedColours: ["BLACK", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "NAVY", "ORANGE", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: ["sprint evo", "sprint-evo", "sprintevo"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "stage-eco": createReferenceEntry({
    slug: "stage-eco",
    displayName: "Stage Eco",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE", "BLACK/ANTHRACITE", "NAVY/LIGHT NAVY"],
    aliases: ["stage eco", "stage-eco", "stageeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "starfall": createReferenceEntry({
    slug: "starfall",
    displayName: "Starfall",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "MINT BOTTLE GREEN", "MINT/BOTTLE GREEN", "NAVY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "steel": createReferenceEntry({
    slug: "steel",
    displayName: "Steel",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "NAVY WHITE", "RED WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "steel-eco": createReferenceEntry({
    slug: "steel-eco",
    displayName: "Steel Eco",
    garmentType: "t-shirt",
    allowedColours: ["BLACK/DARK GREY/WHITE", "NAVY/LIGHT NAVY/WHITE", "RED/DARK RED/WHITE", "ROYAL BLUE/DARK ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["steel eco", "steel-eco", "steeleco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "striped": createReferenceEntry({
    slug: "striped",
    displayName: "Striped",
    garmentType: "polo shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "GREY GUN METAL", "NAVY WHITE", "RED WHITE", "WHITE GUN METAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "strive": createReferenceEntry({
    slug: "strive",
    displayName: "Strive",
    garmentType: "socks",
    allowedColours: ["BLACK", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "subzero": createReferenceEntry({
    slug: "subzero",
    displayName: "Subzero",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY", "STONE GRAY", "STONE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "success": createReferenceEntry({
    slug: "success",
    displayName: "Success",
    garmentType: "hoody",
    allowedColours: ["BLACK GUN METAL", "BLACK/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "summit-eco-custom": createReferenceEntry({
    slug: "summit-eco-custom",
    displayName: "Summit Eco Custom",
    garmentType: "shorts",
    allowedColours: [],
    aliases: ["summit eco custom", "summit-eco-custom", "summitecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "surat": createReferenceEntry({
    slug: "surat",
    displayName: "Surat",
    garmentType: "jacket",
    allowedColours: ["BLACK", "BLUE", "NAVY", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "sylen": createReferenceEntry({
    slug: "sylen",
    displayName: "Sylen",
    garmentType: "t-shirt",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tactic": createReferenceEntry({
    slug: "tactic",
    displayName: "Tactic",
    garmentType: "trousers",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "taiga-xh": createReferenceEntry({
    slug: "taiga-xh",
    displayName: "Taiga Xh",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["taiga xh", "taiga-xh", "taigaxh"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "taiga-xi": createReferenceEntry({
    slug: "taiga-xi",
    displayName: "Taiga Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["taiga xi", "taiga-xi", "taigaxi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tailored-design-to-follow-your-movements": createReferenceEntry({
    slug: "tailored-design-to-follow-your-movements",
    displayName: "Tailored Design To Follow Your Movements",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["tailored design to follow your movements", "tailored-design-to-follow-your-movements", "tailoreddesigntofollowyourmovements"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "talisman": createReferenceEntry({
    slug: "talisman",
    displayName: "Talisman",
    garmentType: "mini ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tecla-eco-custom": createReferenceEntry({
    slug: "tecla-eco-custom",
    displayName: "Tecla Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["tecla eco custom", "tecla-eco-custom", "teclaecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tellurium": createReferenceEntry({
    slug: "tellurium",
    displayName: "Tellurium",
    garmentType: "t-shirt",
    allowedColours: ["BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "RED WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tellurium-eco": createReferenceEntry({
    slug: "tellurium-eco",
    displayName: "Tellurium Eco",
    garmentType: "t-shirt",
    allowedColours: ["GREEN/WHITE", "NAVY/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE"],
    aliases: ["tellurium eco", "tellurium-eco", "telluriumeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tempest": createReferenceEntry({
    slug: "tempest",
    displayName: "Tempest",
    garmentType: "training ball",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tender": createReferenceEntry({
    slug: "tender",
    displayName: "Tender",
    garmentType: "bag",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tess": createReferenceEntry({
    slug: "tess",
    displayName: "Tess",
    garmentType: "referee",
    allowedColours: ["BLACK GOLD", "NEON SKY BLACK", "NEON YELLOW BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tess-eco": createReferenceEntry({
    slug: "tess-eco",
    displayName: "Tess Eco",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["tess eco", "tess-eco", "tesseco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "thallium": createReferenceEntry({
    slug: "thallium",
    displayName: "Thallium",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "GREEN WHITE", "GREEN/WHITE", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE GUN METAL", "WHITE/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "thalos": createReferenceEntry({
    slug: "thalos",
    displayName: "Thalos",
    garmentType: "training top",
    allowedColours: ["BLACK", "BLACK/DARK GREY", "BLUE", "GREEN", "GREEN/DARK GREEN", "NAVY", "NAVY/LIGHT NAVY", "NEON SKY", "NEON SKY/DARK NEON SKY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "thames-hero": createReferenceEntry({
    slug: "thames-hero",
    displayName: "Thames Hero",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE", "BLACK", "GUN METAL", "NAVY"],
    aliases: ["thames hero", "thames-hero", "thameshero"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "thavil": createReferenceEntry({
    slug: "thavil",
    displayName: "Thavil",
    garmentType: "polo shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "RED WHITE", "TURQUOISE GUN METAL", "WHITE SILVER"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "theater": createReferenceEntry({
    slug: "theater",
    displayName: "Theater",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "GREY GUN METAL", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "theater-eco": createReferenceEntry({
    slug: "theater-eco",
    displayName: "Theater Eco",
    garmentType: "hoody",
    allowedColours: [],
    aliases: ["theater eco", "theater-eco", "theatereco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "themis": createReferenceEntry({
    slug: "themis",
    displayName: "Themis",
    garmentType: "t-shirt",
    allowedColours: ["AMBER BLACK", "BLACK WHITE", "BLUE WHITE", "CARDINAL WHITE", "GREEN WHITE", "PURPLE WHITE", "RED WHITE", "TURQUOISE GUN METAL", "WHITE BLUE", "WHITE GREEN", "WHITE GUN METAL", "WHITE RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "themis-eco": createReferenceEntry({
    slug: "themis-eco",
    displayName: "Themis Eco",
    garmentType: "t-shirt",
    allowedColours: ["AMBER/BLACK", "BLACK/WHITE", "CARDINAL/WHITE", "GREEN/WHITE", "PURPLE/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "TURQUOISE/ANTHRACITE", "WHITE/ANTHRACITE", "WHITE/GREEN", "WHITE/RED", "WHITE/ROYAL BLUE"],
    aliases: ["themis eco", "themis-eco", "themiseco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tiamat": createReferenceEntry({
    slug: "tiamat",
    displayName: "Tiamat",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE/NEON SKY", "BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "CARDINAL WHITE", "GREEN WHITE", "GREEN/WHITE", "GUN METAL NEON SKY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tiber": createReferenceEntry({
    slug: "tiber",
    displayName: "Tiber",
    garmentType: "tracksuit",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "GREEN BLACK", "GREEN/BLACK", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "SILVER GUN METAL", "SILVER/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tidal": createReferenceEntry({
    slug: "tidal",
    displayName: "Tidal",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tobol": createReferenceEntry({
    slug: "tobol",
    displayName: "Tobol",
    garmentType: "sr",
    allowedColours: ["BLACK", "NAVY", "RED"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "toque": createReferenceEntry({
    slug: "toque",
    displayName: "Toque",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "totem": createReferenceEntry({
    slug: "totem",
    displayName: "Totem",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK YELLOW", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "INDIGO SKY NAVY", "INDIGO SKY/NAVY", "MINT BOTTLE GREEN", "MINT/BOTTLE GREEN", "NAVY", "NAVY WHITE", "NAVY YELLOW", "NAVY/WHITE", "NAVY/YELLOW", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "STONE GRAY GUN METAL", "STONE GREY/ANTHRACITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "trick": createReferenceEntry({
    slug: "trick",
    displayName: "Trick",
    garmentType: "socks",
    allowedColours: ["BLACK", "COLUMBIA", "GREEN", "NAVY", "RED", "ROYAL", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tridens-pro": createReferenceEntry({
    slug: "tridens-pro",
    displayName: "Tridens Pro",
    garmentType: "finish",
    allowedColours: [],
    aliases: ["tridens pro", "tridens-pro", "tridenspro"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "trip": createReferenceEntry({
    slug: "trip",
    displayName: "Trip",
    garmentType: "bag",
    allowedColours: ["BLACK", "CARDINAL", "GREEN", "NAVY", "RED", "ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "trophy": createReferenceEntry({
    slug: "trophy",
    displayName: "Trophy",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tropical-xi": createReferenceEntry({
    slug: "tropical-xi",
    displayName: "Tropical Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["tropical xi", "tropical-xi", "tropicalxi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "trumpet": createReferenceEntry({
    slug: "trumpet",
    displayName: "Trumpet",
    garmentType: "hoody",
    allowedColours: ["ANTHRACITE", "BLACK/ANTHRACITE", "NAVY/LIGHT NAVY", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tsunami-xi": createReferenceEntry({
    slug: "tsunami-xi",
    displayName: "Tsunami Xi",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["tsunami xi", "tsunami-xi", "tsunamixi"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "tulip": createReferenceEntry({
    slug: "tulip",
    displayName: "Tulip",
    garmentType: "kneepads",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "tungsten-eco-custom": createReferenceEntry({
    slug: "tungsten-eco-custom",
    displayName: "Tungsten Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["tungsten eco custom", "tungsten-eco-custom", "tungstenecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "turbolence": createReferenceEntry({
    slug: "turbolence",
    displayName: "Turbolence",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "turvey": createReferenceEntry({
    slug: "turvey",
    displayName: "Turvey",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "twirl": createReferenceEntry({
    slug: "twirl",
    displayName: "Twirl",
    garmentType: "mixture",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "twister": createReferenceEntry({
    slug: "twister",
    displayName: "Twister",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "ultralight": createReferenceEntry({
    slug: "ultralight",
    displayName: "Ultralight",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "under": createReferenceEntry({
    slug: "under",
    displayName: "Under",
    garmentType: "size 4 light",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "valkyria": createReferenceEntry({
    slug: "valkyria",
    displayName: "Valkyria",
    garmentType: "sr",
    allowedColours: ["ANTHRACITE/NEON CORAL", "BLACK WHITE", "BLACK/WHITE", "BLUE WHITE", "GUN METAL NEON CORAL", "NAVY WHITE", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "vanir": createReferenceEntry({
    slug: "vanir",
    displayName: "Vanir",
    garmentType: "full length zip top",
    allowedColours: ["ANTHRACITE/NEON CORAL", "BLACK RED", "BLACK WHITE", "BLACK/RED", "BLACK/WHITE", "BLUE WHITE", "GREEN NEON GREEN", "GREEN/NEON GREEN", "GUN METAL NEON CORAL", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "RED WHITE", "RED/WHITE", "ROYAL BLUE/WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "vibe": createReferenceEntry({
    slug: "vibe",
    displayName: "Vibe",
    garmentType: "unknown",
    allowedColours: ["BLACK", "NAVY", "STONE GREY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "void": createReferenceEntry({
    slug: "void",
    displayName: "Void",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "BOTTLE GREEN", "CARDINAL", "COLUMBIA", "GREEN", "GUN METAL", "NAVY", "NEON CORAL", "NEON GREEN", "NEON SKY", "NEON YELLOW", "ORANGE", "PINK", "PURPLE", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "volga": createReferenceEntry({
    slug: "volga",
    displayName: "Volga",
    garmentType: "sr",
    allowedColours: ["BLACK WHITE", "NAVY WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "vortex": createReferenceEntry({
    slug: "vortex",
    displayName: "Vortex",
    garmentType: "sr",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "vostok": createReferenceEntry({
    slug: "vostok",
    displayName: "Vostok",
    garmentType: "jacket",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "walker-eco-custom": createReferenceEntry({
    slug: "walker-eco-custom",
    displayName: "Walker Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["walker eco custom", "walker-eco-custom", "walkerecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "walo": createReferenceEntry({
    slug: "walo",
    displayName: "Walo",
    garmentType: "polifiber",
    allowedColours: ["BLACK", "NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "walsh": createReferenceEntry({
    slug: "walsh",
    displayName: "Walsh",
    garmentType: "t-shirt",
    allowedColours: ["WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "waltz": createReferenceEntry({
    slug: "waltz",
    displayName: "Waltz",
    garmentType: "polo shirt",
    allowedColours: ["BLACK", "BLUE", "GREY", "NAVY", "RED", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "waltz-eco": createReferenceEntry({
    slug: "waltz-eco",
    displayName: "Waltz Eco",
    garmentType: "polo shirt",
    allowedColours: ["BLACK", "NAVY", "RED", "ROYAL BLUE", "WHITE"],
    aliases: ["waltz eco", "waltz-eco", "waltzeco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "warden": createReferenceEntry({
    slug: "warden",
    displayName: "Warden",
    garmentType: "t-shirt",
    allowedColours: ["BLUE WHITE", "COLUMBIA NAVY", "PURPLE WHITE", "RED WHITE", "WHITE BLUE", "WHITE RED", "YELLOW BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "warden-eco": createReferenceEntry({
    slug: "warden-eco",
    displayName: "Warden Eco",
    garmentType: "t-shirt",
    allowedColours: ["COLUMBIA/NAVY", "PURPLE/WHITE", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE/RED", "WHITE/ROYAL BLUE", "YELLOW/BLACK"],
    aliases: ["warden eco", "warden-eco", "wardeneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "water-bottle": createReferenceEntry({
    slug: "water-bottle",
    displayName: "Water Bottle",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["water bottle", "water-bottle", "waterbottle"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "widia": createReferenceEntry({
    slug: "widia",
    displayName: "Widia",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "GREEN WHITE", "NAVY WHITE", "RED WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "widia-eco": createReferenceEntry({
    slug: "widia-eco",
    displayName: "Widia Eco",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["widia eco", "widia-eco", "widiaeco"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "willow": createReferenceEntry({
    slug: "willow",
    displayName: "Willow",
    garmentType: "t-shirt",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "windfall": createReferenceEntry({
    slug: "windfall",
    displayName: "Windfall",
    garmentType: "bag",
    allowedColours: ["BLACK"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "wing": createReferenceEntry({
    slug: "wing",
    displayName: "Wing",
    garmentType: "bag",
    allowedColours: ["ROYAL"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "wisp": createReferenceEntry({
    slug: "wisp",
    displayName: "Wisp",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/OFF WHITE", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL BOTTLE GREEN/LIGHT BOTTLE GREEN", "GUN METAL OFF WHITE", "LILAC GUN METAL", "LILAC/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "woman-woman-fit-fit": createReferenceEntry({
    slug: "woman-woman-fit-fit",
    displayName: "Woman Woman Fit Fit",
    garmentType: "unknown",
    allowedColours: [],
    aliases: ["woman woman fit fit", "woman-woman-fit-fit", "womanwomanfitfit"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "wright": createReferenceEntry({
    slug: "wright",
    displayName: "Wright",
    garmentType: "t-shirt",
    allowedColours: ["OFF WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "wright-custom": createReferenceEntry({
    slug: "wright-custom",
    displayName: "Wright Custom",
    garmentType: "match day pullover",
    allowedColours: [],
    aliases: ["wright custom", "wright-custom", "wrightcustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "wyn": createReferenceEntry({
    slug: "wyn",
    displayName: "Wyn",
    garmentType: "unknown",
    allowedColours: [],
    aliases: [],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "wyrm": createReferenceEntry({
    slug: "wyrm",
    displayName: "Wyrm",
    garmentType: "t-shirt",
    allowedColours: ["AMBER BLACK", "AMBER/BLACK", "BLACK RED", "BLACK WHITE", "BLACK YELLOW", "BLACK/RED", "BLACK/WHITE", "BLACK/YELLOW", "BLUE WHITE", "BLUE YELLOW", "BOTTLE GREEN WHITE", "BOTTLE GREEN/WHITE", "CARDINAL ORANGE", "CARDINAL/ORANGE", "COLUMBIA NAVY", "COLUMBIA/NAVY", "GREEN BLACK", "GREEN/BLACK", "NAVY RED", "NAVY WHITE", "NAVY/RED", "NAVY/WHITE", "RED BLACK", "RED BLUE", "RED NAVY", "RED WHITE", "RED/BLACK", "RED/NAVY", "RED/ROYAL BLUE", "RED/WHITE", "ROYAL BLUE/WHITE", "ROYAL BLUE/YELLOW", "WHITE BLACK", "WHITE/BLACK", "YELLOW BLUE", "YELLOW NAVY", "YELLOW/NAVY", "YELLOW/ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "wyvern": createReferenceEntry({
    slug: "wyvern",
    displayName: "Wyvern",
    garmentType: "t-shirt",
    allowedColours: ["BLACK GUN METAL", "BLUE WHITE", "CARDINAL COLUMBIA", "COLUMBIA WHITE", "GREEN WHITE", "GUN METAL NEON RED", "NAVY WHITE", "ORANGE BLACK", "RED WHITE", "WHITE BLUE", "WHITE RED", "WHITE SILVER", "YELLOW BLACK", "YELLOW BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "wyvern-eco": createReferenceEntry({
    slug: "wyvern-eco",
    displayName: "Wyvern Eco",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE/NEON RED", "BLACK/ANTHRACITE", "CARDINAL/COLUMBIA", "COLUMBIA/WHITE", "GREEN/WHITE", "NAVY/WHITE", "ORANGE/BLACK", "RED/WHITE", "ROYAL BLUE/WHITE", "WHITE/RED", "WHITE/ROYAL BLUE", "WHITE/SILVER", "YELLOW/BLACK", "YELLOW/ROYAL BLUE"],
    aliases: ["wyvern eco", "wyvern-eco", "wyverneco"],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "x500": createReferenceEntry({
    slug: "x500",
    displayName: "X500",
    garmentType: "double",
    allowedColours: ["BLACK WHITE", "BLUE WHITE", "BLUE YELLOW", "NAVY WHITE", "ORANGE WHITE", "RED WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "yaruga": createReferenceEntry({
    slug: "yaruga",
    displayName: "Yaruga",
    garmentType: "tracksuit",
    allowedColours: ["ANTHRACITE/OFF WHITE", "BLACK", "BLACK/DARK GREY", "BLUE", "BOTTLE GREEN BOTTLE GREEN", "BOTTLE GREEN/LIGHT BOTTLE GREEN", "CARDINAL", "CARDINAL/DARK CARDINAL", "GUN METAL OFF WHITE", "LILAC GUN METAL", "LILAC/ANTHRACITE", "NAVY", "NAVY/LIGHT NAVY", "RED", "RED/DARK RED", "ROYAL BLUE/DARK ROYAL BLUE", "TURQUOISE NAVY", "TURQUOISE/NAVY"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "york-eco-custom": createReferenceEntry({
    slug: "york-eco-custom",
    displayName: "York Eco Custom",
    garmentType: "t-shirt",
    allowedColours: [],
    aliases: ["york eco custom", "york-eco-custom", "yorkecocustom"],
    source: CATALOGUE_PENDING_SOURCE,
  }),
  "zima": createReferenceEntry({
    slug: "zima",
    displayName: "Zima",
    garmentType: "mixture",
    allowedColours: ["BLACK", "BLUE", "NAVY", "RED", "ROYAL BLUE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "zinc": createReferenceEntry({
    slug: "zinc",
    displayName: "Zinc",
    garmentType: "t-shirt",
    allowedColours: ["BLACK WHITE", "BLACK/DARK GREY/WHITE", "BLUE WHITE", "NAVY WHITE", "NAVY/LIGHT NAVY/WHITE", "NEON CORAL GUN METAL", "NEON CORAL/DARK NEON CORAL/ANTHRACITE", "RED WHITE", "RED/DARK RED/WHITE", "ROYAL BLUE/DARK ROYAL BLUE/WHITE", "WHITE"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
  "zyra": createReferenceEntry({
    slug: "zyra",
    displayName: "Zyra",
    garmentType: "t-shirt",
    allowedColours: ["ANTHRACITE", "BLACK", "BLUE", "GREEN", "GUN METAL", "NAVY", "NEON CORAL", "PINK", "RED", "ROYAL BLUE", "WHITE", "YELLOW"],
    aliases: [],
    source: CATALOGUE_VERIFIED_SOURCE,
  }),
};

export const macronModelReferences = Object.values(macronReferenceData);

// Generic words that must NEVER be treated as Macron models
export const NEVER_A_MODEL = new Set([
  "tracksuit","training","cotton","club","shirt","shorts","pro",
  "hooded","bottle","water","fixed","polo","sock","socks","cap",
  "hat","jacket","coat","hoody","hoodie","sweater","tee","top",
  "bottoms","pants","trousers","gloves","bag","bobble","pom",
  "backpack","rucksack","holdall","beanie","warmer","new","old",
  "long","short","winter","summer","snr","jnr","sr","jr","size",
  "large","medium","small","xl","3d","set","kit","plain",
  "embroidered","printed","black","white","red","green","blue",
  "navy","yellow","orange","purple","pink","grey","gray","royal","one",
  "one size",
]);

export const macronReferenceMap = (() => {
  const out = {};
  for (const ref of macronModelReferences) {
    const slug = ref.slug.toLowerCase();
    const display = ref.displayName.toLowerCase();
    if (!NEVER_A_MODEL.has(slug)) out[slug] = ref;
    if (!NEVER_A_MODEL.has(display)) out[display] = ref;
    for (const alias of ref.aliases || []) {
      if (!NEVER_A_MODEL.has(alias.toLowerCase())) out[alias.toLowerCase()] = ref;
    }
    const firstWord = display.split(/\s+/)[0];
    if (firstWord && !NEVER_A_MODEL.has(firstWord) && !out[firstWord]) {
      out[firstWord] = ref;
    }
  }
  return out;
})();

export { CATALOGUE_PENDING_SOURCE, CATALOGUE_VERIFIED_SOURCE };
